...
import { generateThemeCssVariables, getAllowedBrandFonts, type ThemeMode, validateColorPalette } from "@/lib/white-label/theme";

export { contrastRatio, generateThemeCssVariables, getAllowedBrandFonts, validateColorPalette } from "@/lib/white-label/theme";
...
export function buildEmailBrandingTemplate(params: { branding: TenantBrandingSettings; html: string }) {
...
export async function uploadTenantLogo(tenantId: string, params: { dataUrl: string; contentType: string }) {
  ...
  await adminDb.collection("tenants").doc(tenantId).set({ whiteLabel: { logoUrl }, brand: { logoUrl } }, { merge: true });
...
export async function verifyEmailSender(fromEmail: string) {
  const cleanEmail = String(fromEmail || "").trim().toLowerCase();
  ...
  const dkimValid = flattenedDkim.some((record) => record.toLowerCase().includes("k=rsa") || record.toLowerCase().includes("v=dkim1"));
