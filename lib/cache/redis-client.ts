...
export async function setCached<T>(key: string, value: T, ttlSeconds: number, tags: string[] = []): Promise<void> {
  const redis = await getRedisClient();
  if (!redis) return;
...
export async function warmCache<T>(entries: Array<{ key: string; value: T; ttlSeconds: number; tags?: string[] }>) {
  await Promise.all(entries.map((entry) => setCached(entry.key, entry.value, entry.ttlSeconds, entry.tags || [])));
}

export const cacheKeys = {
  exchangeRates: (baseCurrency: string) => `${CACHE_PREFIX.exchangeRates}:base:${baseCurrency}`,
  taxRates: (tenantId: string, filters: Record<string, string | number | boolean | null | undefined>) => {
    const hash = crypto.createHash("sha256").update(JSON.stringify(filters)).digest("hex").slice(0, 16);
    return `${CACHE_PREFIX.taxRates}:tenant:${tenantId}:filters:${hash}`;
  },
  userPermissions: (tenantId: string, userId: string) => `${CACHE_PREFIX.userPermissions}:tenant:${tenantId}:user:${userId}`,
  dashboardData: (tenantId: string, module: string, view: string) => `${CACHE_PREFIX.dashboardData}:tenant:${tenantId}:module:${module}:view:${view}`,
  searchResults: (tenantId: string, module: string, queryHash: string) => `${CACHE_PREFIX.searchResults}:tenant:${tenantId}:module:${module}:query:${queryHash}`,
