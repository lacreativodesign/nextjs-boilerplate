...
export function validateFile(filename: string, sizeBytes: number): { valid: boolean; error?: string } {
  ...
  if (!ALLOWED_EXTENSIONS.has(ext)) {
    return { valid: false, error: `File type .${ext} is not supported. Allowed: documents, images, presentations, archives` };
  }

  if (sizeBytes > MAX_FILE_SIZE) {
    return { valid: false, error: `File size ${(sizeBytes / 1024 / 1024).toFixed(1)}MB exceeds maximum of 25MB` };
  }
