...
function getEncryptionKey(): Buffer {
  ...
  if (candidate.length !== 32)
    throw new Error('SLACK_TOKEN_ENCRYPTION_KEY must decode to 32 bytes.');
  return candidate;
}
...
export async function createSlackOAuthState(params: {
  ...
  await adminDb
    .collection(SLACK_STATE_COLLECTION)
    .doc(state)
    .set({
      state,
...
async function getTenantFromSlackTeam(
  teamId: string,
): Promise<{ tenantId: string; config: SlackIntegrationConfig } | null> {
  const snap = await adminDb
...
export async function sendDirectMessage(params: {
  ...
  if (!config?.connected || !config.notificationEnabled) return;
  const token = await getBotToken(config);
  const convo = await callSlackApi<{ channel: { id: string } }>(token, 'conversations.open', {
...
async function getSlackUserForBizostoUser(
  tenantId: string,
  userId: string,
): Promise<string | null> {
...
export async function sendBizostoEventNotification(
  params:
    | {
        type: 'task_assigned';
        tenantId: string;
    ...
    if (slackUserId) {
      await sendDirectMessage({
        ...
        blocks: [
          {
            type: 'section',
            text: { type: 'mrkdwn', text: `*New Task Assigned*\n${params.title}` },
          },
          {
            type: 'actions',
            elements: [
...
export async function handleSlackSlashCommand(payload: SlackCommandPayload) {
  ...
  if (!match) {
    return {
      ...
      text: 'Bizosto workspace is not connected for this Slack workspace.',
    };
  }
  ...
  if (match.config.commandEnabled === false) {
    return {
      response_type: 'ephemeral',
      text: 'Slash commands are disabled by your Bizosto admin.',
    };
  }
  ...
  switch (command) {
    case 'task create': {
      ...
      if (!projectId || !title)
        return {
          response_type: 'ephemeral',
          text: 'Usage: /bizosto task create project=<projectId> title="Task title" [priority=medium] [assigned=<userId>]',
        };

      const projectSnap = await adminDb.collection('projects').doc(projectId).get();
      if (!projectSnap.exists)
        return { response_type: 'ephemeral', text: `Project ${projectId} not found.` };
      const project = projectSnap.data() as any;
      if (project.tenantId !== match.tenantId)
        return { response_type: 'ephemeral', text: 'Project is not in your tenant.' };

      const requester = await resolveBizostoUserBySlack(match.tenantId, payload.user_id);
      if (!requester)
        return {
          response_type: 'ephemeral',
          text: 'Link your Slack user to Bizosto profile (slackUserId) before using commands.',
        };
      ...
      await taskRef.set({
        ...
        priority: ['low', 'medium', 'high', 'urgent'].includes(
          String(kv.priority || '').toLowerCase(),
        )
          ? String(kv.priority).toLowerCase()
          : 'medium',
      ...
      await adminDb
        ...
        .doc(projectId)
        .set(
          { totalTasks: admin.firestore.FieldValue.increment(1), updatedAt: now },
          { merge: true },
        );
    ...
    case 'invoice status': {
      if (!args)
        return {
          response_type: 'ephemeral',
          text: 'Usage: /bizosto invoice status <invoice-number>',
        };
      ...
      const [byOrder, byId] = await Promise.all([
        ...
        adminDb
          .collection('invoices')
          .where('tenantId', '==', match.tenantId)
          .where(admin.firestore.FieldPath.documentId(), '==', args)
          .limit(1)
          .get(),
      ]);
      ...
      return {
        response_type: 'ephemeral',
        text: `Invoice ${args}: ${invoice.status || 'unknown'}`,
      };
    }
    case 'leave request': {
      const kv = parseKvArgs(args);
      ...
      if (!startDateRaw || !endDateRaw)
        return {
          response_type: 'ephemeral',
          text: 'Usage: /bizosto leave request type=<type> start=YYYY-MM-DD end=YYYY-MM-DD [reason="text"]',
        };
      ...
      if (
        ...
        return { response_type: 'ephemeral', text: 'Invalid leave dates.' };
    ...
    default:
      return {
        response_type: 'ephemeral',
        text: 'Bizosto commands: /bizosto task create, /bizosto invoice status, /bizosto leave request, /bizosto project status',
      };
  }
}

export async function updateSlackInteractiveAction(params: {
  teamId: string;
  actionId: string;
  value: string;
    ...
    await adminDb.runTransaction(async (tx) => {
      ...
      if (req.leaveType !== 'unpaid') {
        ...
        const balanceRef = adminDb
          ...
          .doc(`${match.tenantId}_${req.employeeId}_${req.leaveType}_${year}`);
  ...
  return {
    text:
      params.actionId === 'bizosto_approve'
        ? 'Request approved in Bizosto.'
        : 'Request rejected in Bizosto.',
