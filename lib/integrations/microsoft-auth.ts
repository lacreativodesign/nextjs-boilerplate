...
function getEncryptionKey(): Buffer {
  ...
  if (!raw) {
    throw new Error(
      'MICROSOFT_OAUTH_TOKEN_ENCRYPTION_KEY is required for Microsoft token storage.',
    );
  }
...
export function getMicrosoftOauthConfig() {
  ...
  const authorityTenant =
    String(process.env.MICROSOFT_OAUTH_AUTHORITY_TENANT || 'common').trim() || 'common';
...
export async function storeMicrosoftOAuthState(input: {
  ...
  await adminDb
    .collection(MICROSOFT_STATE_COLLECTION)
    .doc(input.state)
    .set({
      state: input.state,
...
export async function consumeMicrosoftOAuthState(state: string) {
  ...
  return {
    ...
    returnTo: data.returnTo || '/admin/settings/integrations',
  };
}
...
export async function exchangeMicrosoftCodeForTokens(code: string): Promise<MicrosoftTokenPayload> {
  ...
  if (!response.ok || !data.access_token) {
    throw new Error(
      data.error_description || data.error || 'Failed to exchange Microsoft OAuth code.',
    );
  }
...
export async function getMicrosoftProfile(
  accessToken: string,
): Promise<{ email: string; displayName: string | null; tenantId: string | null }> {
  const response = await fetch(
    `${MICROSOFT_GRAPH_BASE}/me?$select=mail,userPrincipalName,displayName`,
    {
  ...
  await ref.set(
    {
      ...
      scopes: String(params.tokenPayload.scope || '')
        .split(' ')
        .filter(Boolean),
...
export async function getMicrosoftIntegration(
  tenantId: string,
): Promise<MicrosoftIntegrationConfig | null> {
...
export async function getValidMicrosoftAccessToken(tenantId: string): Promise<string> {
  ...
  if (!tokens.refresh_token)
    throw new Error('Microsoft refresh token is unavailable. Reconnect Microsoft 365.');
  ...
  await ref.set(
    {
      scopes: String(mergedTokens.scope || '')
        .split(' ')
        .filter(Boolean),
