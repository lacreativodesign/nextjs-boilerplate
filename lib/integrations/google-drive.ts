...
export async function uploadFileToGoogleDrive(params: {
  ...
  const response = await fetch(
    `${DRIVE_UPLOAD_API}?uploadType=multipart&fields=id,name,webViewLink,webContentLink`,
    {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${token}`,
...
export async function shareDriveFile(params: {
  ...
  const body = params.email
    ? { type: 'user', role, emailAddress: params.email }
    : { type: 'anyone', role };
  await driveRequest(params.tenantId, `/files/${encodeURIComponent(params.fileId)}/permissions`, {
    method: 'POST',
    body: JSON.stringify(body),
  });
  return driveRequest<{ webViewLink?: string }>(
    params.tenantId,
    `/files/${encodeURIComponent(params.fileId)}?fields=webViewLink`,
  );
}
