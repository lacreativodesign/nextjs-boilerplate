...
const getTenantIntegrationRef = (tenantId: string) =>
  adminDb
    .collection('tenants')
...
export async function createXeroOAuthState(params: {
  tenantId: string;
  userUid: string;
  returnTo?: string;
}) {
  ...
  await adminDb
    .collection(XERO_STATE_COLLECTION)
    .doc(state)
    .set({
      state,
...
export async function exchangeXeroCodeForTokens(code: string): Promise<XeroTokenPayload> {
  ...
  if (!response.ok || !data.access_token || !data.refresh_token)
    throw new Error(data.error_description || data.error || 'Xero OAuth exchange failed.');
  return {
...
async function refreshXeroToken(current: XeroTokenPayload): Promise<XeroTokenPayload> {
  ...
  const data = (await response.json().catch(() => ({}))) as any;
  if (!response.ok || !data.access_token || !data.refresh_token)
    throw new Error(data.error_description || data.error || 'Xero token refresh failed.');
  return {
    accessToken: data.access_token,
    refreshToken: data.refresh_token,
    expiresAt: Date.now() + Number(data.expires_in || 1800) * 1000,
...
export async function saveXeroConnection(params: {
  ...
  await getTenantIntegrationRef(params.tenantId).set(
    ...
    } satisfies XeroIntegrationConfig & Record<string, unknown>,
    { merge: true },
  );
}
...
async function getAccessTokenAndOrganization(tenantId: string) {
  ...
  if (!config.connected || !config.tokensEncrypted || !config.organizationId)
    throw new Error('Xero connection is incomplete.');
  const tokenPayload = decryptJson<XeroTokenPayload>(config.tokensEncrypted);
  if (tokenPayload.expiresAt > Date.now() + 60_000)
    return { accessToken: tokenPayload.accessToken, organizationId: config.organizationId };
    ...
    await doc.ref.set(
      {
        xero: {
          ...
          invoiceNumber: getString(created.InvoiceNumber),
          lastUpdatedAt: parseXeroDate(created.UpdatedDateUTC),
        },
        updatedAt: admin.firestore.FieldValue.serverTimestamp(),
      },
      { merge: true },
...
async function syncPaymentsFromXero(
  tenantId: string,
  ...
  for (const payment of payments) {
    ...
    if (
      sinceISO &&
      remoteUpdatedAt &&
      new Date(remoteUpdatedAt).getTime() <= new Date(sinceISO).getTime()
    )
      continue;
    ...
    const existing = await adminDb
      .collection('payments')
      .where('tenantId', '==', tenantId)
      .where('xero.id', '==', xeroId)
      .limit(1)
      .get();
    if (!existing.empty) {
      const current = existing.docs[0].data() as any;
      const localUpdatedAt = parseXeroDate(
        current?.xero?.lastUpdatedAt || current?.updatedAt?.toDate?.()?.toISOString?.(),
      );
      if (
        settings.conflictMode === 'manual' &&
        compareLocalVsRemote(localUpdatedAt, remoteUpdatedAt) > 0
      ) {
        result.skipped += 1;
        conflicts += 1;
        continue;
      ...
      await existing.docs[0].ref.set(
        {
          ...
          status: 'completed',
          paidAt: remoteUpdatedAt,
          updatedAt: admin.firestore.FieldValue.serverTimestamp(),
          xero: { id: xeroId, lastUpdatedAt: remoteUpdatedAt },
        },
        { merge: true },
      );
      result.updated += 1;
...
async function syncContactsFromXero(
  tenantId: string,
  sinceISO: string | null,
  settings: XeroSyncSettings,
) {
  ...
  for (const contact of contacts) {
    ...
    const payload = {
      ...
      xero: {
        id: xeroId,
        contactNumber: getString(contact?.ContactNumber),
    ...
    if (existing.empty) {
      await adminDb
        .collection('clients')
        .add({ tenantId, ...payload, createdAt: admin.firestore.FieldValue.serverTimestamp() });
      result.inserted += 1;
      continue;
    }
    const current = existing.docs[0].data() as any;
...
async function syncContactsToXero(tenantId: string) {
  ...
  for (const doc of local.docs) {
    ...
    const payload = {
      ...
      Phones: data.primaryContactPhone
        ? [{ PhoneType: 'DEFAULT', PhoneNumber: getString(data.primaryContactPhone) }]
        : undefined,
    };
    ...
    await doc.ref.set(
      {
        xero: {
          ...
          contactNumber: getString(created.ContactNumber),
          lastUpdatedAt: parseXeroDate(created.UpdatedDateUTC),
        },
        updatedAt: admin.firestore.FieldValue.serverTimestamp(),
      },
...
async function syncAccountsFromXero(tenantId: string, sinceISO: string | null) {
  ...
  for (const account of accounts) {
    ...
    const payload = {
      ...
      code: getString(account?.Code),
      type: getString(account?.Type),
      status: getString(account?.Status) || 'ACTIVE',
      xero: { id: xeroId, lastUpdatedAt: remoteUpdatedAt },
      updatedAt: admin.firestore.FieldValue.serverTimestamp(),
    };
    if (existing.empty) {
...
async function syncTaxRatesFromXero(tenantId: string, sinceISO: string | null) {
  ...
  for (const rate of rates) {
    ...
    const payload = {
      ...
      rate: Number.isFinite(effective) ? effective : 0,
      enabled: getString(rate?.Status) !== 'DELETED',
      type: 'simple',
      applies_to: 'subtotal',
      xero: { id: xeroId, lastUpdatedAt: remoteUpdatedAt },
      updatedAt: admin.firestore.FieldValue.serverTimestamp(),
...
export async function updateXeroSettings(
  tenantId: string,
  userUid: string,
  patch: Partial<XeroSyncSettings>,
): Promise<XeroSyncSettings> {
...
export async function getXeroSyncLogs(tenantId: string, limit = 50) {
  ...
  const snapshot = await adminDb
    .collection('tenants')
    .doc(normalizeTenantId(tenantId))
    .collection(XERO_LOGS_COLLECTION)
...
export async function runXeroSync(input: {
  tenantId: string;
  userUid: string;
  ...
  try {
    ...
    const invoices = settings.syncInvoicesToXero
      ? await syncInvoicesToXero(tenantId)
      : mapSyncEntityResult();
    const payments = settings.syncPaymentsFromXero
      ? await syncPaymentsFromXero(
          tenantId,
    ...
    if (settings.syncContactsTwoWay) {
      const inbound = await syncContactsFromXero(
        tenantId,
        mode === 'initial' ? null : integration.cursor?.contactsLastSyncAt || null,
        settings,
      );
