...
export function getCalendlyOAuthConfig() {
  ...
  if (!clientId || !clientSecret)
    throw new Error('CALENDLY_CLIENT_ID and CALENDLY_CLIENT_SECRET are required.');
  return {
    clientId,
    clientSecret,
...
export async function createCalendlyOAuthState(params: {
  tenantId: string;
  userUid: string;
  returnTo?: string;
}) {
  ...
  await adminDb
    .collection(CALENDLY_STATE_COLLECTION)
    .doc(state)
    .set({
      state,
...
async function fetchCurrentCalendlyUser(accessToken: string) {
  ...
  if (!me?.resource?.uri || !me?.resource?.current_organization)
    throw new Error('Unable to resolve Calendly account metadata.');
  return {
...
export async function ensureCalendlyWebhookSubscription(params: {
  tenantId: string;
  signingKey: string;
  userUid: string;
}) {
  const integration = await getCalendlyIntegration(params.tenantId);
  if (!integration?.organizationUri || !integration?.userUri)
    throw new Error('Calendly connection metadata is missing.');

  const token = await getCalendlyAccessToken(params.tenantId);
  const callbackUrl = `${getBaseUrl()}/api/integrations/calendly/webhook`;

  const response = await calendlyRequest<{ resource?: { uri?: string } }>(
    token,
...
export async function updateCalendlySettings(
  ...
  patch: Partial<
    Pick<
      CalendlyConnection,
      'defaultProjectId' | 'widgetUrl' | 'syncEnabled' | 'createTasksFromMeetings'
    >
  >,
) {
  await integrationRef(tenantId).set(
    {
      ...
      createTasksFromMeetings:
        patch.createTasksFromMeetings === undefined
          ? undefined
          : Boolean(patch.createTasksFromMeetings),
      updatedAt: admin.firestore.FieldValue.serverTimestamp(),
    },
    { merge: true },
  );
...
export async function listCalendlyEvents(tenantId: string, limit = 50) {
  const snap = await meetingsCollection(tenantId)
...
export async function upsertMeetingFromCalendlyWebhook(params: { tenantId: string; payload: any }) {
  ...
  const existingSnap = await meetingsCollection(params.tenantId)
  ...
  const status =
    invitee.status === 'canceled' ? 'canceled' : invitee.rescheduled ? 'rescheduled' : 'scheduled';
  ...
  const baseRecord = {
    ...
    endTime: String(
      event.end_time || invitee.end_time || event.start_time || new Date().toISOString(),
    ),
...
export async function cancelCalendlyMeeting(params: {
  tenantId: string;
  meetingId: string;
  reason?: string;
}) {
...
export async function syncCalendlyEventsFromApi(tenantId: string, count = 20) {
  const integration = await getCalendlyIntegration(tenantId);
  if (!integration?.syncEnabled || !integration.userUri)
    throw new Error('Calendly sync is not enabled.');
