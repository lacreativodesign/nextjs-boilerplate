...
async function outlookRequest<T>(tenantId: string, path: string, init: RequestInit = {}): Promise<T> {
...
export async function sendEmailViaOutlook(params: {
  ...
  const trackingRef = params.trackOpens
    ? adminDb.collection("tenants").doc(params.tenantId).collection("integrations").doc("microsoft365").collection("outlookTracking").doc()
    : null;
  ...
  await outlookRequest<unknown>(params.tenantId, "/me/sendMail", {
    ...
    body: JSON.stringify({
      message: {
        ...
        internetMessageHeaders: params.threadId ? [{ name: "X-Bizosto-Thread-Id", value: params.threadId }] : undefined,
...
export async function markOutlookTrackingOpen(trackingId: string) {
  const query = await adminDb.collectionGroup("outlookTracking").where("trackingId", "==", trackingId).limit(1).get();
