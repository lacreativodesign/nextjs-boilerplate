...
export async function storeGoogleOAuthState(input: {
  ...
  await adminDb
    .collection(GOOGLE_STATE_COLLECTION)
    .doc(input.state)
    .set({
      state: input.state,
...
export async function getGoogleIntegration(
  tenantId: string,
): Promise<GoogleIntegrationConfig | null> {
...
export async function getValidGoogleAccessToken(tenantId: string): Promise<string> {
  ...
  if (!response.ok || !refreshed?.access_token) {
    throw new Error(
      refreshed?.error_description || refreshed?.error || 'Google token refresh failed.',
    );
  }
