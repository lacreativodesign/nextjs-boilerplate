...
export async function upsertBizostoEventToOutlook(params: {
  tenantId: string;
  calendarId?: string;
  ...
  const payload: Record<string, unknown> = {
    ...
    location: params.event.locationDisplayName
      ? { displayName: params.event.locationDisplayName }
      : undefined,
  ...
  const apiPathPrefix = params.calendarId
    ? `/me/calendars/${encodeURIComponent(params.calendarId)}`
    : '/me';
  const method = outlookEventId ? 'PATCH' : 'POST';
  const path = outlookEventId
    ? `${apiPathPrefix}/events/${encodeURIComponent(outlookEventId)}`
    : `${apiPathPrefix}/events`;

  const result = await graphCalendarRequest<{
    id: string;
    webLink?: string;
...
export async function importOutlookCalendarEvents(params: {
  ...
  const basePath = params.calendarId
    ? `/me/calendars/${encodeURIComponent(params.calendarId)}/events`
    : '/me/events';
    ...
    events:
      data.value?.map((item) => {
        const externalBizostoEventId =
          item.singleValueExtendedProperties?.find((entry) => entry.id?.includes('bizostoEventId'))
            ?.value || null;
        return {
...
export async function listOutlookCalendars(tenantId: string) {
  return graphCalendarRequest<{
    value?: Array<{ id: string; name: string; canEdit?: boolean; isDefaultCalendar?: boolean }>;
  }>(tenantId, '/me/calendars?$top=100');
}
