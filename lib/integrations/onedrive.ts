...
async function oneDriveRequest<T>(
  tenantId: string,
  path: string,
  init: RequestInit = {},
): Promise<T> {
...
export async function ensureOneDriveFolderMapping(params: {
  ...
  const parentPath = params.parentOneDriveFolderId
    ? `/me/drive/items/${encodeURIComponent(params.parentOneDriveFolderId)}/children`
    : '/me/drive/root/children';
  const created = await oneDriveRequest<{ id: string; name?: string }>(
    params.tenantId,
    parentPath,
    {
      method: 'POST',
...
export async function shareOneDriveFile(params: {
  tenantId: string;
  fileId: string;
...
export async function listOneDriveFolders(params: { tenantId: string; parentFolderId?: string }) {
  ...
  const data = await oneDriveRequest<{
    value?: Array<{
      id?: string;
      name?: string;
      webUrl?: string;
