...
function interpolate(input: unknown, context: WorkflowExecutionContext): unknown {
  ...
  return input.replace(/\{\{\s*([^}]+)\s*\}\}/g, (_, rawPath) => {
    ...
    const value = getValue({ record: context.record || {}, previousRecord: context.previousRecord || {} }, path);
    return value == null ? "" : String(value);
  });
}

function compareCondition(condition: WorkflowCondition, context: WorkflowExecutionContext): boolean {
...
async function insertRunLog(runId: string, tenantId: string, log: WorkflowRunLog) {
  await adminDb.collection("automation_workflow_runs").doc(runId).collection("logs").add({ ...log, tenantId });
}

async function resolveApprovers(step: ApprovalStep, tenantId: string, actorUid?: string): Promise<string[]> {
  if (step.type === "user") return step.userIds || [];
  if (step.type === "role") {
    const roles = step.roles || [];
    if (!roles.length) return [];
    const usersSnap = await adminDb.collection("users").where("tenantId", "==", tenantId).where("role", "in", roles).get();
    return usersSnap.docs.map((doc) => doc.id);
  }
  if (step.type === "group") {
    const ids = step.groupIds || [];
    if (!ids.length) return [];
    const groupSnaps = await Promise.all(ids.map((id) => adminDb.collection("approval_groups").doc(id).get()));
    return groupSnaps.flatMap((snap) => (snap.exists ? ((snap.data()?.memberUids as string[]) || []) : []));
  }
...
async function executeAction(action: WorkflowAction, context: WorkflowExecutionContext, runId: string): Promise<{ awaitingApproval?: boolean }> {
  switch (action.type) {
    case "create_record": {
      const payload = Object.entries(action.payload || {}).reduce<Record<string, unknown>>((acc, [key, value]) => {
    ...
    case "approval": {
      ...
      await approvalRef.set({
        ...
        expiresAt: action.expiresInHours ? new Date(Date.now() + action.expiresInHours * 60 * 60 * 1000).toISOString() : null,
        createdAt: new Date().toISOString(),
      });
...
export class WorkflowEngine {
  async execute(workflow: WorkflowDefinition, context: WorkflowExecutionContext) {
    ...
    if (!triggerMatches(workflow, context)) {
      ...
      await runRef.set({ status: "failed", completedAt: new Date().toISOString(), error: "Trigger mismatch" }, { merge: true });
    ...
    if (!evaluateConditions(workflow.conditions || [], context)) {
      ...
      await runRef.set({ status: "success", completedAt: new Date().toISOString() }, { merge: true });
      ...
      try {
        for (const action of workflow.actions || []) {
          ...
          if (result.awaitingApproval) {
            await runRef.set({ status: "awaiting_approval", completedAt: new Date().toISOString(), retries }, { merge: true });
    ...
    await runRef.set({ status: "failed", completedAt: new Date().toISOString(), error: "Retry limit reached" }, { merge: true });
    return { runId: runRef.id, status: "failed" as WorkflowRunStatus };
  }
}
