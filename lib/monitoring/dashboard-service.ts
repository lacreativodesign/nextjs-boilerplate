...
import type { MetricIngestPayload, MonitoringDashboardPayload, MonitoringModule } from "@/lib/monitoring/types";
...
function toModule(value: string | undefined): MonitoringModule {
  if (value === "api" || value === "database" || value === "cache" || value === "auth" || value === "billing" || value === "frontend") {
    return value;
  }
  return "unknown";
}
...
export async function getMonitoringDashboard(): Promise<MonitoringDashboardPayload> {
  ...
  const [usageLogs, metricEvents] = await Promise.all([getUsageLogs(dayAgo), getMetricEvents(dayAgo)]);
  ...
  const endpointMap = new Map<string, { responseTimes: number[]; errors: number; requests: number }>();
  ...
  for (const log of usageLogs) {
    ...
    const endpointStats = endpointMap.get(endpoint) || { responseTimes: [], errors: 0, requests: 0 };
  ...
  const cacheHits = metricEvents.filter((event) => event.type === "cache_event" && event.cacheResult === "hit").length;
  const cacheMisses = metricEvents.filter((event) => event.type === "cache_event" && event.cacheResult === "miss").length;
  ...
  const cacheHitRate = round(cacheHits + cacheMisses > 0 ? (cacheHits / (cacheHits + cacheMisses)) * 100 : 0);
