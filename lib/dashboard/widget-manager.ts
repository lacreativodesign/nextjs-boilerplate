...
async function getRevenueChartData(tenantId: string, config: Record<string, unknown>) {
  ...
  try {
    ...
    for (const doc of snap.docs) {
      ...
      const issuedAt = invoice.issuedAt?.toDate
        ? invoice.issuedAt.toDate()
        : new Date(invoice.issuedAt || 0);
      const idx = monthStarts.findIndex(
        (start, position) =>
          issuedAt >= start &&
          (position === monthStarts.length - 1 || issuedAt < monthStarts[position + 1]),
      );
      if (idx >= 0) {
...
async function getTaskSummaryData(tenantId: string, config: Record<string, unknown>) {
  ...
  try {
    ...
    for (const doc of snap.docs) {
      const task = doc.data();
      const done =
        Boolean(task.completed) || String(task.status || '').toLowerCase() === 'completed';
      if (done) {
        completed += 1;
        continue;
      }
