...
function buildFinanceTools(): AgentTool[] {
  const readTools = TOOL_REGISTRY.filter(
    (t) => t.access === "read_only" && ["coo", "admin", "finance"].some((a) => t.allowedAgents.includes(a))
  );
...
function validateFinanceToolCall(toolName: string, tools: AgentTool[]): { tool?: AgentTool; error?: string } {
  ...
  if (tool.access === "write") {
    ...
    if (!validation.valid) return { error: validation.error || `Tool ${toolName} is not permitted` };
  }
...
export async function runFinanceAgent(taskId: string, tenantId: string): Promise<void> {
  ...
  try {
    ...
    const executeReadTool = async (toolName: string, input: JsonObject): Promise<unknown> => {
      ...
      const body = json && typeof json === "object" ? json as { ok?: boolean; error?: string; result?: unknown } : {};
    ...
    const interceptWriteTool = (toolName: string, input: JsonObject): unknown => {
      if (toolName === "send_payment_reminder") {
        ...
        return { queued: true, actionId: action.id, message: "Payment reminder queued for human approval." };
      }
    ...
    if (keyConfig.provider === "anthropic") {
      result = await runAnthropicFinanceLoop(
        ...
        toolCalls, executeReadTool, interceptWriteTool,
    ...
    await markTaskFailed(taskId, err instanceof Error ? err.message : "Finance agent execution failed");
  }
}
...
async function runAnthropicFinanceLoop(
  ...
  for (let i = 0; i < MAX_ITERATIONS; i++) {
    ...
    if (data.stop_reason === "tool_use") {
      const toolUseBlocks = data.content.filter((b): b is { type: "tool_use"; id: string; name: string; input?: JsonObject } => b.type === "tool_use");
      ...
      for (const block of toolUseBlocks) {
        ...
        if (validation.error || !validation.tool) {
          ...
          toolCalls.push({ toolName: block.name, input: block.input || {}, output: toolOutput, executedAt: new Date().toISOString() });
        }
...
async function runOpenAIFinanceLoop(
  ...
  for (let i = 0; i < MAX_ITERATIONS; i++) {
    const res = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({ model: "gpt-4o", max_tokens: 2048, tools: openaiTools, tool_choice: "auto", messages }),
    });
    ...
    if (choice?.finish_reason === "tool_calls") {
      for (const tc of message.tool_calls || []) {
        ...
        if (validation.error || !validation.tool) {
          ...
          toolCalls.push({ toolName, input: toolInput, output: toolOutput, executedAt: new Date().toISOString() });
        }
