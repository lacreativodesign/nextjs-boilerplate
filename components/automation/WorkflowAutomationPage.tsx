...
export default function WorkflowAutomationPage() {
  ...
  const [approvals, setApprovals] = useState<Array<{ id: string; status: string; workflowId: string; mode: string }>>([]);
  ...
  const addCondition = () => {
    const next: WorkflowCondition = { id: crypto.randomUUID(), field: "", operator: "eq", value: "" };
  ...
  const addAction = (type: WorkflowAction["type"]) => {
    const base: WorkflowAction =
      type === "create_record"
        ...
        : type === "send_email"
        ? { id: crypto.randomUUID(), type, to: ["ops@bizosto.com"], subject: "Workflow Notice", body: "Action completed" }
        : type === "webhook"
        ? { id: crypto.randomUUID(), type, url: "https://example.internal/webhook", method: "POST" }
        : { id: crypto.randomUUID(), type, mode: "sequential", steps: [{ id: crypto.randomUUID(), type: "manager" }] };
      ...
      <div className="card" style={{ padding: 20, borderRadius: 16 }}>
        <div className="flex items-center justify-between">
          <div>
            ...
            <p style={{ color: "var(--text-muted)", fontSize: 13 }}>Build and run event, schedule, and manual workflows with approvals.</p>
          </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[320px_1fr]">
        <section className="card" style={{ padding: 16, borderRadius: 16 }}>
          <div className="mb-3 flex items-center justify-between">
            ...
            <button className="btn" onClick={() => setSelected({ ...EMPTY_WORKFLOW })}>New</button>
          </div>
          {loading ? (
            ...
            <div className="space-y-2">
              {workflows.map((item) => (
                <button
                  ...
                  style={{ borderColor: "var(--border-subtle)", background: selectedId === item.id ? "var(--surface-muted)" : "transparent" }}
                  onClick={() => setSelected(item)}
                >
                  <div className="flex items-center justify-between gap-2">
                    <div style={{ fontWeight: 600 }}>{item.name}</div>
                    <span style={{ background: statusTone(item.status).bg, color: statusTone(item.status).fg, borderRadius: 999, padding: "2px 8px", fontSize: 11 }}>
                      {item.status}
                    </span>
                  </div>
                  <div style={{ color: "var(--text-muted)", fontSize: 12 }}>{item.trigger?.type}</div>
                </button>
              ))}
            </div>
          )}
          ...
          <div className="mt-4">
            <div style={{ fontSize: 12, color: "var(--text-muted)", marginBottom: 6 }}>Templates</div>
            <div className="flex flex-wrap gap-2">
              {templates.map((template) => (
                <button key={template} className="btn ghost" onClick={() => setSelected((prev) => ({ ...prev, templateKey: template }))}>
                  {template}
                </button>
              ))}
            </div>
          ...
          <div className="card" style={{ padding: 16, borderRadius: 16 }}>
            ...
            <div className="mt-3 grid gap-3 md:grid-cols-2">
              <label>
                ...
                <input className="input" value={selected.name || ""} onChange={(e) => setSelected((prev) => ({ ...prev, name: e.target.value }))} />
              </label>
              <label>
                ...
                <select
                  ...
                  onChange={(e) => setSelected((prev) => ({ ...prev, status: e.target.value as WorkflowDefinition["status"] }))}
                >
            ...
            <div className="mt-4 grid gap-4 md:grid-cols-3">
              <div className="rounded-xl border p-3">
                ...
                <select
                  ...
                  onChange={(e) => setSelected((prev) => ({ ...prev, trigger: { type: e.target.value as WorkflowTrigger["type"] } as WorkflowTrigger }))}
                >
              ...
              <div className="rounded-xl border p-3 md:col-span-2">
                <div className="mb-2 flex items-center justify-between">
                  ...
                  <button className="btn ghost" onClick={addCondition}>Add condition</button>
                </div>
                <div className="space-y-2">
                    ...
                    <div key={condition.id} className="grid grid-cols-3 gap-2">
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setSelected((prev) => {
                            const conditions = [...(prev.conditions || [])];
                            conditions[index] = { ...conditions[index], operator: e.target.value as WorkflowCondition["operator"] };
                            return { ...prev, conditions };
                          })
                        }
                      >
            ...
            <div className="mt-4 rounded-xl border p-3">
              <div className="mb-2 flex items-center justify-between">
                ...
                <div className="flex flex-wrap gap-2">
                  {(["create_record", "update_field", "delete_record", "send_email", "webhook", "approval"] as const).map((type) => (
                  ...
                  <div key={action.id} className="rounded-lg border p-2">
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        className="btn ghost"
                        onClick={() =>
                          setSelected((prev) => ({ ...prev, actions: (prev.actions || []).filter((item) => item.id !== action.id) }))
                        }
                      >
                        Remove
                      </button>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              <button className="btn" onClick={saveWorkflow} disabled={saving}>{saving ? "Saving..." : "Save workflow"}</button>
              {selected.id && <button className="btn ghost" onClick={() => toggleWorkflow(selected.id!)}>Enable/Disable</button>}
              {selected.id && <button className="btn ghost" onClick={testRun}>Test run</button>}
            </div>
          </div>
                ...
                <div key={approval.id} className="rounded-xl border p-3">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <div style={{ fontSize: 12, color: "var(--text-muted)" }}>Workflow {approval.workflowId}</div>
                    </div>
                    <div className="flex gap-2">
                      <button className="btn ghost" onClick={() => respondApproval(approval.id, "approve")}>Approve</button>
                      <button className="btn ghost" onClick={() => respondApproval(approval.id, "reject")}>Reject</button>
                    </div>
                  </div>
                </div>
              ))}
              {!approvals.length && <div style={{ color: "var(--text-muted)", fontSize: 13 }}>No pending automation approvals.</div>}
            </div>
          </div>
              ...
              {runs.map((run) => (
                <div key={run.id} className="rounded-xl border p-3">
                  <div className="flex items-center justify-between">
                    ...
                    <span style={{ background: statusTone(run.status).bg, color: statusTone(run.status).fg, borderRadius: 999, padding: "2px 8px", fontSize: 11 }}>
                      {run.status}
                    </span>
                  </div>
                  <div style={{ fontSize: 12, color: "var(--text-muted)" }}>{run.startedAt} → {run.completedAt || "running"}</div>
                  {run.error && <div style={{ color: "var(--danger)", fontSize: 12 }}>{run.error}</div>}
                  <div className="mt-2 rounded-lg bg-[var(--surface-muted)] p-2 text-xs">
                    {(run.logs || []).slice(-4).map((log, index) => (
                      <div key={`${run.id}-${index}`}>{log.ts} [{log.level}] {log.message}</div>
                    ))}
                  </div>
                </div>
              ))}
              {!runs.length && <div style={{ color: "var(--text-muted)", fontSize: 13 }}>No execution history.</div>}
            </div>
          </div>
        </section>
      </div>
