  ...
  const renderNavItem = (item: NavItem, depth = 0) => {
    ...
    return (
      <div key={item.id} className="w-full">
        <div className="flex items-center">
          {hasChildren ? (
            ...
            >
              ...
              <span className={`flex-1 truncate transition-all ${effectiveCollapsed ? "md:opacity-0 md:group-hover:opacity-100" : "opacity-100"}`}>
                {item.label}
              </span>
              <ChevronDown
                className={`h-4 w-4 transition-transform ${isExpanded ? "rotate-180" : ""} ${
                  effectiveCollapsed ? "md:opacity-0 md:group-hover:opacity-100" : "opacity-100"
                }`}
              />
            </button>
          ) : (
            ...
            >
              ...
              {item.badge && (
                <span
                  className={`rounded-full px-2 py-0.5 text-xs font-semibold ${badgeStyles[item.badge.variant]} ${
                    effectiveCollapsed ? "md:opacity-0 md:group-hover:opacity-100" : "opacity-100"
                  }`}
                >
                  {item.badge.text}
        ...
        {hasChildren && (
          <div
            className={`ml-12 grid gap-1 overflow-hidden transition-all ${
              isExpanded && !effectiveCollapsed ? "max-h-96 opacity-100" : "max-h-0 opacity-0"
            }`}
          >
  ...
  return (
    <>
      ...
      <button
        ...
        onClick={closeMobile}
        className={`fixed inset-0 z-40 bg-[var(--drawer-overlay)] transition-opacity md:hidden ${
      ...
      <aside
        className={`sidebar-transition group fixed inset-y-0 left-0 z-50 flex w-[var(--sidebar-width)] flex-col border-r border-[var(--border-subtle)] bg-[var(--surface-card)] shadow-lg md:shadow-none ${
          ...
          effectiveCollapsed
            ? "md:w-[var(--sidebar-collapsed-width)] md:hover:w-[var(--sidebar-width)]"
      ...
      >
        ...
        <div className="flex items-center gap-3 px-4 py-4">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[var(--surface-muted)] text-sm font-bold">
            BZ
          </div>
        ...
        <div className="border-t border-[var(--border-subtle)] px-4 py-4">
          <div className="flex items-center gap-3">
            <div className="flex h-11 w-11 items-center justify-center rounded-full bg-[var(--surface-muted)] text-sm font-semibold">
            ...
            <button
              ...
              className={`hidden h-10 w-10 items-center justify-center rounded-xl border border-[var(--border-subtle)] bg-[var(--surface-card)] text-[var(--text-primary)] md:flex ${
                effectiveCollapsed ? "md:opacity-100" : ""
              }`}
          ...
          <button
            ...
            className={`mt-3 flex w-full items-center gap-2 rounded-xl border border-[var(--border-subtle)] px-3 py-2 text-sm text-[var(--danger)] ${
              effectiveCollapsed ? "md:justify-center" : ""
            }`}
          >
            <LogOut className="h-4 w-4" />
            <span className={`${effectiveCollapsed ? "md:hidden md:group-hover:inline" : "inline"}`}>Logout</span>
          </button>
        </div>
      </aside>
    </>
