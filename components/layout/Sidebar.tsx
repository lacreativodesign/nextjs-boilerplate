...
export default function Sidebar({
  ...
  const navItems = getNavigationForTenant(currentRole, tenantModules).map(
    (item) => ({
      ...item,
      label: item.labelKey
  ...
  const groups = groupNavigationItems(navItems);
  ...
  const showSubscriberHelp = currentRole !== "client";
          ...
          <nav className="workspace-sidebar__nav">
            {groups.map((group) => (
              <div key={group.id} className="workspace-sidebar__group">
                <p className="workspace-sidebar__group-label sidebar-label">
                  {group.label}
                </p>
