...
export default function SalesAgentWidget() {
  ...
  const [actionStates, setActionStates] = useState<
    Record<string, 'idle' | 'loading' | 'done' | 'rejected' | 'error'>
  >({});
    ...
    try {
      const createRes = await apiFetch('/api/ai/agent-tasks', {
        ...
        body: JSON.stringify({
          agentType: 'sales',
          prompt:
            "Review our full sales pipeline. Identify stalling leads that haven't progressed in 7+ days. Summarise pipeline health by stage and total value. Then propose specific stage updates for the most critical stalling leads.",
        }),
      });
  ...
  const handleReject = (action: SalesProposedAction) => {
    ...
    const remaining = task?.proposedActions?.filter(
      (a) => a.status === 'pending' && a.id !== action.id && actionStates[a.id] !== 'rejected',
    );
    ...
    <div className="rounded-xl border border-[var(--border-subtle)] bg-[var(--surface-card)] p-5">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-sm font-semibold text-[var(--text-primary)] flex items-center gap-2">
            <BarChart3 className="h-4 w-4 text-[var(--erp-blue)]" /> Sales Agent
          </p>
      ...
      {(widgetState === 'creating' || widgetState === 'processing') && (
        <div className="mt-4 flex items-center gap-3">
          ...
          <p className="text-sm text-[var(--text-muted)]">
            {widgetState === 'creating'
              ? 'Initialising sales agent...'
              : 'Scanning pipeline for stalling leads...'}
          </p>
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  ...
                  {state === 'loading' && (
                    <p className="text-xs text-[var(--text-muted)] flex-shrink-0">Applying...</p>
                  )}
                  {state === 'done' && (
                  ...
                  {state === 'rejected' && (
                    <span className="text-xs text-[var(--text-muted)] flex-shrink-0">Rejected</span>
                  )}
                  {state === 'error' && (
                    <span className="text-xs font-semibold text-red-400 flex-shrink-0">Failed</span>
                  )}
                </div>
              </div>
            );
      ...
      {resolvedActions.length > 0 && (
        <div className="mt-3 space-y-1">
          {resolvedActions.map((action) => (
            ...
            >
              <span>
                {action.status === 'executed' || actionStates[action.id] === 'done' ? '✓' : '✗'}
              </span>
              <span>
                {action.label} —{' '}
                {action.status === 'executed' || actionStates[action.id] === 'done'
      ...
      {widgetState === 'idle' && (
        <p className="mt-3 text-xs text-[var(--text-muted)]">
          Scans your full pipeline, identifies leads stalling for 7+ days, and proposes stage
          updates for your approval before applying any changes.
        </p>
      )}
