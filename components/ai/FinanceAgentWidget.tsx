    ...
    try {
      const createRes = await apiFetch('/api/ai/agent-tasks', {
        ...
        body: JSON.stringify({
          agentType: 'finance',
          prompt:
            'Analyse our current financial health. Check overdue invoices, review revenue and payment trends, summarise cash flow status. Then propose payment reminder emails for the most urgent overdue invoices.',
        }),
      });
      const createJson = (await createRes.json().catch(() => ({}))) as CreateTaskResponse;
      if (!createRes.ok || !createJson.ok || !createJson.task?.id)
    ...
    <div className="rounded-xl border border-[var(--border-subtle)] bg-[var(--surface-card)] p-5">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-sm font-semibold text-[var(--text-primary)] flex items-center gap-2">
            <TrendingUp className="h-4 w-4 text-[var(--erp-blue)]" /> Finance Agent
          </p>
          <p className="mt-0.5 text-xs text-[var(--text-muted)]">
            Analyses cash flow · proposes payment reminders
          </p>
        </div>
        ...
        <div className="mt-4 flex items-center gap-3">
          ...
          <p className="text-sm text-[var(--text-muted)]">
            {widgetState === 'creating'
              ? 'Initialising finance agent...'
              : 'Analysing invoices, revenue, and cash flow...'}
          </p>
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  ...
                  {state === 'loading' && (
                    <p className="text-xs text-[var(--text-muted)] flex-shrink-0">Sending...</p>
                  )}
                  {state === 'done' && (
        ...
        <div className="mt-3 space-y-1.5">
          {resolvedActions.map((action) => (
            ...
            >
              ...
              <span>
                {action.label} — {action.status === 'executed' ? 'sent' : 'rejected'}
              </span>
            </div>
          ))}
        </div>
      ...
      {widgetState === 'idle' && (
        <p className="mt-3 text-xs text-[var(--text-muted)]">
          Analyses overdue invoices, cash flow, and revenue trends. Proposes payment reminders for
