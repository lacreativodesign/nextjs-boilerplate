  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const createRes = await fetch("/api/ai/agent-tasks", {
        ...
        body: JSON.stringify({
          agentType: "coo",
          prompt: "Generate my daily business summary. Check revenue, overdue invoices, open leads, active projects, and team overview. Then give me a clear summary of the business health and what needs attention today.",
        }),
      });
    ...
    <div id="ai-summary-widget" className="rounded-xl border border-[var(--border-subtle)] bg-[var(--surface-card)] p-5">
      ...
      {state === "idle" && (
        <p className="mt-3 text-xs text-[var(--text-muted)]">
          Click Generate to get a plain-English summary of your business health, overdue items, and what needs attention today.
        </p>
      )}
    </div>
  );
