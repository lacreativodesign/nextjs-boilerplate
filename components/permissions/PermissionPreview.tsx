    ...
    <div className="card" style={{ padding: 16 }}>
      ...
      {!snapshot ? (
        ...
        <div className="space-y-2">
          <div className="text-xs">Roles: {snapshot.roles.map((role) => role.name).join(", ") || "None"}</div>
          {snapshot.permissions.map((permission, index) => (
            <div key={`${permission.module}-${permission.entity}-${index}`} className="rounded border p-2 text-xs">
