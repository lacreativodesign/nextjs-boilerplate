...
export function ProjectCard({ project }: ProjectCardProps) {
  return (
    <Link href={`/dashboard/projects/${project.id}`} className="card block p-4 transition hover:shadow-[var(--shadow-lift)]">
      <div className="mb-2 flex items-center justify-between">
        <h3 className="font-semibold">{project.name}</h3>
        <span className="rounded-lg bg-[var(--surface-muted)] px-2 py-1 text-xs uppercase text-[var(--text-muted)]">{project.status}</span>
      </div>
