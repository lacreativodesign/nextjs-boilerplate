...
function WidgetBody({ widget, payload }: { widget: DashboardWidget; payload?: WidgetDataEnvelope }) {
  const data = payload?.data || {};
  switch (widget.type) {
    case "revenue_chart": {
      const series = (data.series as Array<{ month: string; revenue: number }> | undefined) || [];
      return <div className="space-y-1 text-sm">{series.map((row) => <div key={row.month} className="flex justify-between"><span>{row.month}</span><span>{formatCurrency(row.revenue)}</span></div>)}</div>;
    }
    case "invoice_status": {
      const segments = (data.segments as Array<{ name: string; value: number }> | undefined) || [];
      return <div className="space-y-1 text-sm">{segments.map((row) => <div key={row.name} className="flex justify-between"><span>{row.name}</span><span>{row.value}</span></div>)}</div>;
    }
    case "task_summary":
    case "project_status": {
      const bars = (data.bars as Array<{ name?: string; status?: string; value: number }> | undefined) || [];
      return <div className="space-y-1 text-sm">{bars.map((row) => <div key={row.name || row.status} className="flex justify-between"><span>{row.name || row.status}</span><span>{row.value}</span></div>)}</div>;
    }
    case "top_clients": {
      const rows = (data.rows as Array<{ client: string; revenue: number }> | undefined) || [];
      return <div className="space-y-1 text-sm">{rows.map((row) => <div key={row.client} className="flex justify-between"><span className="truncate">{row.client}</span><span>{formatCurrency(row.revenue)}</span></div>)}</div>;
    }
    case "recent_activity": {
      const entries = (data.entries as Array<{ id: string; label: string; actor: string }> | undefined) || [];
      return <ul className="list-disc space-y-1 pl-4 text-sm">{entries.map((entry) => <li key={entry.id}>{entry.label} · {entry.actor}</li>)}</ul>;
    }
    case "time_tracking":
      return <div className="text-2xl font-semibold">{String(data.hours || 0)}h</div>;
    case "leave_calendar": {
      const leaves = (data.leaves as Array<{ id: string; employee: string; status: string }> | undefined) || [];
    ...
    (widgetId: string, deltaX: number, deltaY: number) => {
      ...
      const next = layout.items.map((item) =>
        item.widgetId === widgetId
          ? { ...item, x: Math.max(0, Math.min(COLUMNS - item.w, item.x + dx)), y: Math.max(0, item.y + dy) }
          : item
      );
