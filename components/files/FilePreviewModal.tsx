...
export function FilePreviewModal({
  ...
  open: boolean;
  onClose: () => void;
  file?: { name: string; mimeType: string; previewUrl?: string };
}) {
    ...
    >
      ...
      >
        ...
        {isPdf && file.previewUrl ? (
          <iframe src={file.previewUrl} className="h-[75vh] w-full" />
        ) : null}
        {isVideo && file.previewUrl ? (
          <video controls src={file.previewUrl} className="max-h-[75vh] w-full" />
        ) : null}
      </div>
    </div>
  );
