...
export function FileBrowser({ files, onOpenPreview, onOpenVersions, onRefresh }: {
  files: FileRecord[];
      ...
      <div className={view === "grid" ? "grid grid-cols-1 gap-3 md:grid-cols-2" : "space-y-2"}>
        {filtered.map((file) => (
          <div key={file.id} className="rounded border p-3">
            ...
            <div className="text-xs text-gray-500">v{file.latestVersion} • {(file.size / (1024 * 1024)).toFixed(2)} MB</div>
            <div className="mt-1 text-xs">{file.tags.join(", ")}</div>
            <div className="mt-2 flex gap-2">
              <button className="btn ghost" onClick={() => onOpenPreview(file)}>Preview</button>
              <button className="btn ghost" onClick={() => onOpenVersions(file.id)}>Versions</button>
            </div>
          </div>
        ))}
      </div>
