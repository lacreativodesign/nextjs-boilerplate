...
export default function PeriodSelector({
  ...
  const options =
    periodType === "monthly" ? getLast12Months() : periodType === "quarterly" ? getLast8Quarters() : getLast5Years();
  ...
  return (
    <div className="card p-4 flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
      ...
      <select className="input w-full sm:w-56" value={period} onChange={(e) => onPeriodChange(e.target.value)}>
        {options.map((opt) => (
