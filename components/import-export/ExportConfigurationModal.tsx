...
export default function ExportConfigurationModal({ open, availableFields, onClose, onSubmit }: Props) {
  const [selected, setSelected] = useState<string[]>(availableFields.slice(0, 5).map((field) => field.sourceField));
    ...
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/40 p-4">
      <div className="w-full max-w-lg rounded-lg bg-white p-4 dark:bg-neutral-900">
          ...
          {availableFields.map((field) => (
            <label key={field.sourceField} className="flex items-center gap-2 text-sm">
              <input
                ...
                onChange={(event) => {
                  setSelected((prev) =>
                    event.target.checked ? [...prev, field.sourceField] : prev.filter((entry) => entry !== field.sourceField)
                  );
                }}
              />
              {field.label}
        ...
        <div className="mt-4">
          ...
          <select className="mt-1 w-full rounded border p-2 text-sm" value={format} onChange={(event) => setFormat(event.target.value as "csv" | "excel")}>
