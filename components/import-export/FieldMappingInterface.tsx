...
export default function FieldMappingInterface({ sourceColumns, destinationFields, value, onChange }: Props) {
  const mappedBySource = useMemo(() => new Map(value.map((item) => [item.sourceColumn, item])), [value]);
  const [activeSource, setActiveSource] = useState<string | null>(null);

  const updateMapping = (source: string, destination: string) => {
    const next = value.filter((item) => item.sourceColumn !== source && item.destinationField !== destination);
          ...
          {sourceColumns.map((column) => (
            <button
              ...
              className={`flex w-full items-center justify-between rounded px-3 py-2 text-left text-sm ${
                activeSource === column ? "bg-blue-100 dark:bg-blue-900/30" : "bg-neutral-100 dark:bg-neutral-800"
              }`}
            >
              <span>{column}</span>
              <span className="text-xs text-[var(--text-muted)]">{mappedBySource.get(column)?.destinationField || "unmapped"}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="rounded border p-3">
        ...
        <div className="space-y-2">
          {destinationFields.map((field) => (
            ...
            >
              <span>{field}</span>
              <span className="text-xs text-[var(--text-muted)]">{value.find((item) => item.destinationField === field)?.sourceColumn || "unmapped"}</span>
            </button>
          ))}
        </div>
      </div>
