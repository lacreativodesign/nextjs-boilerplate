...
function HighlightText({ text, tokens }: { text: string; tokens: string[] }) {
  ...
  return (
    <>
      {text.slice(0, idx)}
      <mark className="rounded bg-yellow-200/80 px-0.5 text-[var(--text-primary)]">
      ...
      <div className="modal-scale-enter w-full max-w-4xl rounded-2xl border border-[var(--border-subtle)] bg-[var(--surface-card)] shadow-2xl">
        ...
        {!query.trim() && recents.length > 0 ? (
          <div className="max-h-[60vh] overflow-auto p-4">
            <div className="mb-3 text-xs font-semibold uppercase tracking-wide text-[var(--text-soft)]">
              Recent searches
            </div>
          ...
          <div className="max-h-[60vh] overflow-auto p-4">
            ...
            {!loading && results.length === 0 ? (
              <div className="text-sm text-[var(--text-soft)]">No results found.</div>
            ) : null}
                  ...
                  {items.map((item) => (
                    ...
                    >
                      ...
                      <div className="text-xs text-[var(--text-soft)]">
                        <HighlightText
                          text={item.subtitle || item.highlights.join(' … ')}
                          tokens={queryTokens}
                        />
                      </div>
                    </button>
