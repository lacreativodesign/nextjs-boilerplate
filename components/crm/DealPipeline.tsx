          ...
          <section key={stage} className="card p-4">
            ...
            <div className="space-y-2">
              {stageDeals.map((deal) => (
                <div
                  key={deal.id}
                  className="rounded-xl border border-[var(--border-subtle)] bg-[var(--surface-muted)] p-3 text-sm"
                >
                  <p className="font-medium text-[var(--text-primary)]">
                    {deal.name || deal.title}
                  </p>
                  <p className="text-[var(--text-muted)]">
                    ${(deal.value || deal.valueUSD || 0).toLocaleString()}
                  </p>
                </div>
              ))}
              {stageDeals.length === 0 && (
                <p className="text-xs text-[var(--text-soft)]">No deals</p>
              )}
            </div>
          </section>
        );
