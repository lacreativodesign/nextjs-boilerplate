...
export function ReportCard({ report, onRun }: { report: Report | unknown; onRun: () => void }) {
  return (
    <div className="card flex h-full flex-col justify-between gap-4 p-5">
      <div>
        <div className="flex items-start justify-between gap-2">
          <div>
            <h3 className="text-lg font-semibold text-[var(--text-primary)]">{(report as Record<string, unknown>).name}</h3>
            {(report as Record<string, unknown>).description ? (
              <p className="mt-1 text-sm text-[var(--text-muted)]">{(report as Record<string, unknown>).description}</p>
            ) : null}
          </div>
          <span className="rounded-full bg-[var(--surface-muted)] px-2 py-1 text-xs font-medium text-[var(--text-muted)]">
            {(report as Record<string, unknown>).type === "preset" ? "Preset" : "Custom"}
          </span>
        </div>
        <div className="mt-4 flex flex-wrap gap-2 text-xs text-[var(--text-muted)]">
          <span className="rounded-full bg-[var(--erp-blue-soft)] px-2 py-1 text-[var(--erp-blue)]">
            {(report as Record<string, unknown>).category}
          </span>
          <span className="rounded-full bg-[var(--surface-muted)] px-2 py-1 text-[var(--text-muted)]">
            {(report as Record<string, unknown>).dataSource}
          </span>
          {(report as Record<string, unknown>).isScheduled ? (
