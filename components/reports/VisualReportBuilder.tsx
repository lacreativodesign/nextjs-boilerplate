...
const templateSeeds = [
  { name: "Sales pipeline by stage", dataSource: "deals", groupBy: ["stage"], aggField: "id", aggFunction: "count" },
  {
  ...
  { name: "Time tracking by employee", dataSource: "users", groupBy: ["displayName"], aggField: "trackedHours", aggFunction: "sum" },
  {
  ...
  { name: "Overdue invoices summary", dataSource: "invoices", groupBy: ["status"], aggField: "id", aggFunction: "count" },
  { name: "Project status overview", dataSource: "projects", groupBy: ["status"], aggField: "id", aggFunction: "count" },
] as const;
...
export function VisualReportBuilder() {
  ...
  const addFilter = () => {
    setFilters((prev) => [...prev, { field: availableFields[0] || "status", operator: "equals", value: "" }]);
  };
  ...
  const saveReport = async () => {
    ...
    await fetch("/api/reports/custom", {
      ...
      body: JSON.stringify({
        ...
        category: "custom",
        dataSource,
        fields: selectedFields.map((field, index) => ({ field, label: field, selectedAt: index + 1 })),
        filters,
        groupBy,
        aggregations: [{ field: selectedFields[selectedFields.length - 1] || "id", function: "count", alias: "value" }],
    ...
    <div className="space-y-6 p-6">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-[var(--text-muted)]">Build multi-module custom reports with filters, grouping, charts, exports, and schedules.</p>
        </div>
        <div className="flex gap-2">
          <button className="rounded bg-blue-600 px-3 py-2 text-sm text-white" onClick={saveReport}>Save report</button>
          <button className="rounded bg-neutral-200 px-3 py-2 text-sm dark:bg-neutral-800" onClick={() => setScheduleModalOpen(true)} disabled={!activeReportId}>Schedule</button>
          <button className="rounded bg-neutral-200 px-3 py-2 text-sm dark:bg-neutral-800" onClick={exportReport} disabled={!activeReportId}>Export</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        <aside className="rounded border border-neutral-200 p-4 dark:border-neutral-800">
          ...
          <select className="mb-3 w-full rounded border p-2 text-sm" value={dataSource} onChange={(e) => setDataSource(e.target.value as DataSource)}>
        ...
        >
          ...
          <input value={name} onChange={(event) => setName(event.target.value)} className="mb-3 w-full rounded border p-2 text-sm" placeholder="Report name" />
          <div className="mb-3 flex flex-wrap gap-2">
            {selectedFields.length ? selectedFields.map((field) => (
              <span key={field} className="rounded bg-blue-100 px-2 py-1 text-xs text-blue-700 dark:bg-blue-900/40 dark:text-blue-200">{field}</span>
            )) : <span className="text-xs text-[var(--text-muted)]">Drag fields here</span>}
          </div>

          <h3 className="text-xs font-semibold uppercase text-[var(--text-muted)]">Filter builder</h3>
          <div className="mt-2 space-y-2">
            {filters.map((filter, index) => (
              <div key={`${filter.field}-${index}`} className="grid grid-cols-3 gap-2">
                <select
                  value={filter.field}
                  onChange={(event) => setFilters((prev) => prev.map((row, i) => (i === index ? { ...row, field: event.target.value } : row)))}
                ...
                <select
                  ...
                  onChange={(event) => setFilters((prev) => prev.map((row, i) => (i === index ? { ...row, operator: event.target.value as ReportFilter["operator"] } : row)))}
                  className="rounded border p-2 text-xs"
                >
                ...
                <input
                  ...
                  onChange={(event) => setFilters((prev) => prev.map((row, i) => (i === index ? { ...row, value: event.target.value } : row)))}
                  className="rounded border p-2 text-xs"
                />
              </div>
            ))}
            <button className="rounded border px-2 py-1 text-xs" onClick={addFilter}>+ Add filter</button>
          </div>

          <div className="mt-4">
            <button className="rounded bg-blue-600 px-3 py-2 text-xs text-white" disabled={!activeReportId} onClick={() => runPreview()}>Run preview</button>
          </div>
        </main>
        ...
        <aside className="rounded border border-neutral-200 p-4 dark:border-neutral-800">
          ...
          <select className="mb-3 w-full rounded border p-2 text-sm" value={groupBy[0] || ""} onChange={(event) => setGroupBy(event.target.value ? [event.target.value] : [])}>
          ...
          <select className="mb-3 w-full rounded border p-2 text-sm" value={chartType} onChange={(event) => setChartType(event.target.value as Report["chartType"])}>
          ...
          <select className="w-full rounded border p-2 text-sm" value={exportFormat} onChange={(event) => setExportFormat(event.target.value as ReportFormat)}>
          ...
          <div className="mt-6">
            <h3 className="mb-2 text-xs font-semibold uppercase text-[var(--text-muted)]">Pre-built templates</h3>
            <div className="space-y-2">
              {templateSeeds.map((template) => (
                <button key={template.name} className="block w-full rounded border px-2 py-1 text-left text-xs" onClick={() => applyTemplate(template.name)}>
                  {template.name}
                </button>
              ))}
            </div>
      ...
      <section className="rounded border border-neutral-200 p-4 dark:border-neutral-800">
        <div className="mb-2 flex items-center justify-between">
          ...
          <button className="rounded border px-2 py-1 text-xs" onClick={loadReports}>Refresh</button>
        </div>
        <div className="grid grid-cols-1 gap-2 md:grid-cols-2">
          {reports.map((report) => (
            ...
            >
              ...
              <p className="text-xs text-[var(--text-muted)]">{report.dataSource} · {report.chartType || "table"}</p>
            </button>
          ))}
        </div>
      </section>
          ...
          <div className="max-h-80 overflow-auto rounded border">
            <table className="min-w-full text-xs">
              <thead className="bg-neutral-100 dark:bg-neutral-900">
                <tr>
                  {Object.keys(previewRows[0]).map((key) => <th key={key} className="px-2 py-1 text-left">{key}</th>)}
                </tr>
              </thead>
              <tbody>
                {previewRows.map((row, index) => (
                  <tr key={index} className="border-t border-neutral-200 dark:border-neutral-800">
                    {Object.keys(previewRows[0]).map((key) => <td key={key} className="px-2 py-1">{String(row[key] ?? "")}</td>)}
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="w-full max-w-lg rounded bg-white p-4 dark:bg-neutral-950">
            ...
            <div className="grid grid-cols-2 gap-3">
              <select
                ...
                onChange={(event) => setSchedule((prev) => ({ ...prev, frequency: event.target.value as ReportSchedule["frequency"] }))}
              >
                <option value="daily">Daily</option>
                <option value="weekly">Weekly</option>
                <option value="monthly">Monthly</option>
              </select>
              <input type="time" className="rounded border p-2 text-sm" value={schedule.time} onChange={(event) => setSchedule((prev) => ({ ...prev, time: event.target.value }))} />
              <input className="rounded border p-2 text-sm" placeholder="Timezone" value={schedule.timezone} onChange={(event) => setSchedule((prev) => ({ ...prev, timezone: event.target.value }))} />
              <input className="rounded border p-2 text-sm" placeholder="Recipients comma separated" value={scheduleRecipients} onChange={(event) => setScheduleRecipients(event.target.value)} />
            </div>
            <div className="mt-4 flex justify-end gap-2">
              <button className="rounded border px-3 py-2 text-sm" onClick={() => setScheduleModalOpen(false)}>Cancel</button>
              <button className="rounded bg-blue-600 px-3 py-2 text-sm text-white" onClick={scheduleReport}>Save schedule</button>
            </div>
          </div>
        </div>
      ) : null}
