...
export default function QualityAssuranceWorkspace() {
  ...
  const severityCounts = useMemo(() => {
    return severityOptions.reduce((acc, s) => { acc[s] = defects.filter((d) => d.severity === s).length; return acc; }, {} as Record<DefectSeverity, number>);
  }, [defects]);

  const statusCounts = useMemo(() => {
    return statusOptions.reduce((acc, s) => { acc[s] = defects.filter((d) => d.status === s).length; return acc; }, {} as Record<DefectStatus, number>);
  }, [defects]);
    ...
    <div className="space-y-6">
      ...
      <div className="kpis">
        <div className="card kpi-card">
          ...
          <p className="text-3xl font-bold text-[var(--text-primary)]">{metrics?.totalDefects ?? 0}</p>
        </div>
        <div className="card kpi-card">
        ...
        <div className="card kpi-card">
          ...
          <p className="text-3xl font-bold text-[var(--text-primary)]">{metrics?.defectDensity ?? 0}</p>
        </div>
        <div className="card kpi-card">
          <p className="helper-text mb-1">Escape Rate</p>
          <p className="text-3xl font-bold text-[var(--erp-blue)]">{metrics?.defectEscapeRate ?? 0}%</p>
        </div>
        <div className="card kpi-card">
          <p className="helper-text mb-1">Mean Aging (days)</p>
          <p className="text-3xl font-bold text-[var(--text-primary)]">{metrics?.meanAgingDays ?? 0}</p>
        </div>
        <div className="card kpi-card">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div className="card space-y-3">
          ...
          {severityOptions.map((s) => (
            <BarRow key={s} label={s.toUpperCase()} value={severityCounts[s] || 0} max={Math.max(1, defects.length)} color="var(--danger)" />
          ))}
        </div>
        <div className="card space-y-3">
          <h2 className="section-title">By Status</h2>
          {statusOptions.map((s) => (
            <BarRow key={s} label={s.replace("_", " ")} value={statusCounts[s] || 0} max={Math.max(1, defects.length)} color="var(--erp-blue)" />
          ))}
        </div>
      </div>
      ...
      {Object.keys(trends).length > 0 && (
        <div className="card space-y-3">
          ...
          {Object.entries(trends).sort(([a], [b]) => a.localeCompare(b)).map(([month, count]) => (
            <BarRow key={month} label={month} value={count} max={Math.max(...Object.values(trends), 1)} color="var(--erp-blue)" />
          ))}
        </div>
      )}
      ...
      <div className="card space-y-4">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          ...
          <div className="flex gap-2 flex-wrap">
            <select className="input" style={{ width: "auto" }} value={severityFilter} onChange={(e) => setSeverityFilter(e.target.value)}>
          ...
          <div className="table-shell">
            <div>
              <table>
                ...
                <tbody>
                  {defects.map((d) => (
                    <tr key={d.id} onClick={() => setSelectedDefectId(d.id)} className="cursor-pointer">
                      <td className="font-medium">{d.title}</td>
                      <td><span className={`rounded-full px-2 py-0.5 text-xs font-semibold ${severityColor[d.severity] || ""}`}>{d.severity}</span></td>
                      <td><span className={`rounded-full px-2 py-0.5 text-xs font-semibold ${statusColor[d.status] || ""}`}>{d.status.replace("_", " ")}</span></td>
        ...
        <div className="card space-y-3">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div>
              ...
              <select className="input" value={editDefect.status} onChange={(e) => setEditDefect((p) => ({ ...p, status: e.target.value as DefectStatus }))}>
                {statusOptions.map((s) => <option key={s} value={s}>{s}</option>)}
              </select>
            </div>
            <div>
              <label className="field-label text-sm mb-1 block">Severity</label>
              <select className="input" value={editDefect.severity} onChange={(e) => setEditDefect((p) => ({ ...p, severity: e.target.value as DefectSeverity }))}>
                {severityOptions.map((s) => <option key={s} value={s}>{s}</option>)}
              </select>
            </div>
            <div>
              <label className="field-label text-sm mb-1 block">Root Cause Category</label>
              <input className="input" value={editDefect.rootCauseCategory} onChange={(e) => setEditDefect((p) => ({ ...p, rootCauseCategory: e.target.value }))} placeholder="Root cause category" />
            </div>
            <div>
              <label className="field-label text-sm mb-1 block">Root Cause Notes</label>
              <textarea className="input" rows={2} value={editDefect.rootCauseNotes} onChange={(e) => setEditDefect((p) => ({ ...p, rootCauseNotes: e.target.value }))} placeholder="Root cause analysis notes" />
            </div>
          </div>
          <label className="flex items-center gap-2 text-sm">
            <input type="checkbox" checked={editDefect.escapedToProduction} onChange={(e) => setEditDefect((p) => ({ ...p, escapedToProduction: e.target.checked }))} />
            Escaped to production
          </label>
          <p className="helper-text">Aging: {selectedDefect.defectAgingDays} days | SLA due: {selectedDefect.slaDueAt || "N/A"}</p>
          <button className="btn" onClick={() => void saveDefectUpdate()}>Update Defect</button>
        </div>
      )}
      ...
      <div className="card space-y-3">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <div className="sm:col-span-2">
            ...
            <input className="input" value={newDefect.title} onChange={(e) => setNewDefect((p) => ({ ...p, title: e.target.value }))} placeholder="Defect title" />
          </div>
          <div className="sm:col-span-2">
            <label className="field-label text-sm mb-1 block">Description</label>
            <textarea className="input" rows={3} value={newDefect.description} onChange={(e) => setNewDefect((p) => ({ ...p, description: e.target.value }))} placeholder="Describe the defect" />
          </div>
          <div>
            <label className="field-label text-sm mb-1 block">Type</label>
            <select className="input" value={newDefect.type} onChange={(e) => setNewDefect((p) => ({ ...p, type: e.target.value as DefectType }))}>
              {typeOptions.map((t) => <option key={t} value={t}>{t}</option>)}
            </select>
          </div>
          <div>
            <label className="field-label text-sm mb-1 block">Severity</label>
            <select className="input" value={newDefect.severity} onChange={(e) => setNewDefect((p) => ({ ...p, severity: e.target.value as DefectSeverity }))}>
              {severityOptions.map((s) => <option key={s} value={s}>{s}</option>)}
            </select>
          </div>
          <div>
            <label className="field-label text-sm mb-1 block">Requirement ID</label>
            <input className="input" value={newDefect.requirementId} onChange={(e) => setNewDefect((p) => ({ ...p, requirementId: e.target.value }))} placeholder="Requirement ID (optional)" />
          </div>
          <div className="flex items-center gap-2 text-sm pt-5">
            <input type="checkbox" checked={newDefect.escapedToProduction} onChange={(e) => setNewDefect((p) => ({ ...p, escapedToProduction: e.target.checked }))} />
            <label>Escaped to production</label>
          </div>
        </div>
        <button className="btn" onClick={() => void submitDefect()}>Submit Defect</button>
      </div>
      ...
      <div className="card space-y-3">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <div>
            <label className="field-label text-sm mb-1 block">Title</label>
            <input className="input" value={newTestCase.title} onChange={(e) => setNewTestCase((p) => ({ ...p, title: e.target.value }))} placeholder="Test case title" />
          </div>
          <div>
            <label className="field-label text-sm mb-1 block">Requirement ID</label>
            <input className="input" value={newTestCase.requirementId} onChange={(e) => setNewTestCase((p) => ({ ...p, requirementId: e.target.value }))} placeholder="Requirement ID" />
          </div>
          <div className="sm:col-span-2">
            <label className="field-label text-sm mb-1 block">Description</label>
            <textarea className="input" rows={2} value={newTestCase.description} onChange={(e) => setNewTestCase((p) => ({ ...p, description: e.target.value }))} placeholder="Description" />
          </div>
          <div>
            <label className="field-label text-sm mb-1 block">Steps (one per line)</label>
            <textarea className="input" rows={3} value={newTestCase.steps} onChange={(e) => setNewTestCase((p) => ({ ...p, steps: e.target.value }))} placeholder="One step per line" />
          </div>
          <div>
            <label className="field-label text-sm mb-1 block">Expected Result</label>
            <textarea className="input" rows={3} value={newTestCase.expectedResult} onChange={(e) => setNewTestCase((p) => ({ ...p, expectedResult: e.target.value }))} placeholder="Expected result" />
          </div>
        </div>
        <button className="btn" onClick={() => void submitTestCase()}>Create Test Case</button>
      ...
      <div className="card space-y-3">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <div>
            ...
            <select className="input" value={newTestRun.testCaseId} onChange={(e) => setNewTestRun((p) => ({ ...p, testCaseId: e.target.value }))}>
          ...
          <div>
            ...
            <select className="input" value={newTestRun.status} onChange={(e) => setNewTestRun((p) => ({ ...p, status: e.target.value as "passed" | "failed" | "blocked" }))}>
          ...
          <div className="sm:col-span-2">
            ...
            <textarea className="input" rows={2} value={newTestRun.notes} onChange={(e) => setNewTestRun((p) => ({ ...p, notes: e.target.value }))} placeholder="Run notes" />
          </div>
        </div>
        <button className="btn" onClick={() => void submitTestRun()}>Execute Test Run</button>
      </div>
