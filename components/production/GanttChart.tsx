...
function fmtLabel(d: Date, zoom: ZoomLevel) {
  ...
  if (zoom === "week") return `W${Math.ceil(d.getDate() / 7)} ${d.toLocaleDateString(undefined, { month: "short" })}`;
...
export default function GanttChart({
  ...
  const [dragging, setDragging] = useState<{ taskId: string; startX: number; initialStart: string; initialEnd: string } | null>(null);
  ...
  const arrows = dependencies
    .map((dep) => {
      ...
      return { id: dep.id, path: `M ${fromX} ${y1} L ${midX} ${y1} L ${midX} ${y2} L ${toX} ${y2}` };
    })
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <button className="px-3 py-1 text-xs rounded-md border border-gray-300" onClick={exportPng}>Export PNG</button>
          <button className="px-3 py-1 text-xs rounded-md border border-gray-300" onClick={exportPdf}>Export PDF</button>
        </div>
      </div>

      <div className="rounded-lg border border-gray-200 overflow-auto" ref={containerRef} onMouseMove={onMouseMove} onMouseUp={onMouseUp}>
        <div style={{ minWidth: totalDays * pixelPerDay + 200 }}>
          <div className="sticky top-0 bg-white z-10 border-b border-gray-200" style={{ height: 36 }}>
            {Array.from({ length: totalDays }).map((_, idx) => {
              const date = new Date(minDate);
              date.setUTCDate(date.getUTCDate() + idx);
              return (
                <span key={idx} className="absolute text-[10px] text-gray-500" style={{ left: idx * pixelPerDay + 160, top: 10 }}>
                  {fmtLabel(date, zoom)}
                </span>
              );
            })}
          ...
          <div className="relative" style={{ height: localTasks.length * ROW_HEIGHT + 24 }}>
            <svg width={totalDays * pixelPerDay + 220} height={localTasks.length * ROW_HEIGHT + 24} className="absolute inset-0 pointer-events-none">
              <defs>
              ...
              {arrows.map((arrow) => (
                <path key={arrow.id} d={arrow.path} stroke="#6b7280" strokeWidth="1.2" fill="none" markerEnd="url(#arrowhead)" />
              ))}
            </svg>

            {positionedTasks.map((task) => (
              <div key={task.id} className="absolute flex items-center" style={{ top: task.y, left: 0, height: 22 }}>
                ...
                <button
                  ...
                  onMouseDown={(event) =>
                    setDragging({ taskId: task.id, startX: event.clientX, initialStart: task.startDate, initialEnd: task.endDate })
                  }
                >
                  {task.milestone ? "◆" : `${task.progress || 0}%`}
                  <span className="absolute left-0 top-0 h-full bg-white/30" style={{ width: `${Math.min(100, task.progress || 0)}%` }} />
                </button>
              </div>
            ))}
            ...
            {milestones
              ...
              .map((milestone) => {
                ...
                return (
                  <div key={milestone.id} className="absolute top-0 h-full pointer-events-none" style={{ left: x }}>
                    <div className="h-full border-l border-dashed border-amber-500" />
                    <div className="absolute top-1 -left-2 text-[10px] text-amber-700">{milestone.name}</div>
                  </div>
                );
              })}
          </div>
