...
export default function LeaveManagementDashboard({ canApprove }: { canApprove: boolean }) {
  ...
  return (
    <div style={{ display: "grid", gap: 16 }}>
      ...
      {error ? <div style={{ background: "#fee2e2", color: "#7f1d1d", padding: 12, borderRadius: 8 }}>{error}</div> : null}
      ...
      <section style={cardStyle}>
        ...
        <form onSubmit={handleSubmit} style={{ display: "grid", gap: 12, gridTemplateColumns: "repeat(auto-fit,minmax(220px,1fr))" }}>
          ...
          <button type="submit" style={{ ...inputStyle, cursor: "pointer", background: "#111827", color: "#fff", maxWidth: 220 }}>
            Submit Request
          </button>
        </form>
      </section>
        ...
        {loading ? (
          ...
          <div style={{ display: "grid", gap: 10, gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))" }}>
            {balances.map((balance) => (
              <div key={balance.id} style={{ border: "1px solid #e5e7eb", borderRadius: 8, padding: 10 }}>
      ...
      <section style={cardStyle}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(7,minmax(0,1fr))", gap: 6 }}>
          {monthCells.map((day) => {
            ...
            return (
              <div key={iso} style={{ border: "1px solid #e5e7eb", borderRadius: 8, minHeight: 90, padding: 6, background: isCurrentMonth ? "#fff" : "#f9fafb" }}>
                <div style={{ fontSize: 12, marginBottom: 4 }}>{day.getUTCDate()}</div>
                {entries.slice(0, 2).map((entry) => (
                  <div key={entry.id} style={{ fontSize: 11, borderRadius: 6, padding: "2px 6px", background: "#dbeafe", marginBottom: 3 }}>
        ...
        {pendingQueue.length === 0 ? (
          ...
          <div style={{ display: "grid", gap: 8 }}>
            {pendingQueue.map((req) => (
              <div key={req.id} style={{ border: "1px solid #e5e7eb", borderRadius: 8, padding: 10 }}>
                <div>
        ...
        <div style={{ display: "grid", gap: 8 }}>
          {requests.map((req) => (
            <div key={req.id} style={{ border: "1px solid #e5e7eb", borderRadius: 8, padding: 10 }}>
              ...
              {req.rejectionReason ? <div style={{ color: "#991b1b" }}>Reason: {req.rejectionReason}</div> : null}
            </div>
          ))}
        </div>
      </section>
