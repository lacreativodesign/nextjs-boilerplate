...
async function gzipSizeKb(relativeAssetPath) {
  const normalized = relativeAssetPath.startsWith("/") ? relativeAssetPath.slice(1) : relativeAssetPath;
...
async function evaluateBuildManifest() {
  ...
  const largestRoute = routeReports.reduce((max, current) => (current.kb > max.kb ? current : max), { route: "n/a", kb: 0 });
...
async function main() {
  try {
    ...
    throw new Error("Build manifests are missing. Run `npm run build` before checking bundle size.");
  }
  ...
  if (report.mainBundleKb > MAX_MAIN_BUNDLE_KB) {
    violations.push(`Main bundle ${report.mainBundleKb.toFixed(2)}KB exceeds ${MAX_MAIN_BUNDLE_KB}KB.`);
  }
  if (report.firstLoadJsKb > MAX_FIRST_LOAD_JS_KB) {
    violations.push(`First Load JS ${report.firstLoadJsKb.toFixed(2)}KB exceeds ${MAX_FIRST_LOAD_JS_KB}KB.`);
  }
