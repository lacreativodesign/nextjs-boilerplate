...
import { Bell, FileText, FolderKanban, LayoutDashboard, LogOut, Menu, User } from "lucide-react";
...
const navItems = [
  ...
  { label: "Files", path: "/client/files", icon: FileText },
  { label: "Billing", path: "/client/billing", icon: FileText },
  { label: "Change Requests", path: "/client/change-requests", icon: FileText },
...
export default function ClientLayout({ children }: { children: React.ReactNode }) {
  ...
  const handleNotificationClick = async (item: NotificationItem) => {
    ...
    if (item.entityType === "change_request") {
      router.push("/client/change-requests");
      return;
    }
  ...
  if (!authInstance) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-gray-50 dark:bg-[#0f0f11] text-gray-900 dark:text-gray-100">
        <p className="text-sm font-medium">{authError || "Loading client console…"}</p>
      </div>
    );
  }
      ...
      <div className="admin-shell flex min-h-screen transition-colors">
        ...
        >
          ...
          <nav className="flex flex-col gap-1 px-2">
            {navItems.map((item) => {
              const active = current === item.path || (item.path !== "/client" && current.startsWith(item.path));
        ...
        {drawerOpen && (
          <div className="drawer-overlay" onClick={() => setDrawerOpen(false)}>
            <div className="drawer-panel drawer-panel--md notification-drawer" onClick={(event) => event.stopPropagation()}>
