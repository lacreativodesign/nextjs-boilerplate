  ...
  return (
    <div className="page-frame space-y-8">
      ...
      <section className="rounded border p-4">
        ...
        <form className="grid grid-cols-1 gap-3 md:grid-cols-4" onSubmit={onCreatePolicy}>
          <input
            ...
            onChange={(e) => setPolicyForm((v) => ({ ...v, entityType: e.target.value }))}
          />
          <input
            className="rounded border p-2"
            placeholder="Collection path"
            value={policyForm.collectionPath}
            onChange={(e) => setPolicyForm((v) => ({ ...v, collectionPath: e.target.value }))}
          />
          <input
            className="rounded border p-2"
            type="number"
            ...
            onChange={(e) =>
              setPolicyForm((v) => ({ ...v, retentionDays: Number(e.target.value) }))
            }
          />
          <select
            className="rounded border p-2"
            ...
            onChange={(e) =>
              setPolicyForm((v) => ({ ...v, action: e.target.value as 'delete' | 'archive' }))
            }
          >
            <option value="archive">Archive</option>
            <option value="delete">Delete</option>
          </select>
          <button className="rounded bg-black px-3 py-2 text-white md:col-span-4">
            Create policy
          </button>
        </form>
        <p className="mt-4 text-sm">
          Total policies: {policySummary.total} | Archive: {policySummary.archiveCount} | Delete:{' '}
          {policySummary.deleteCount}
        </p>
      </section>
      ...
      <section className="grid gap-4 md:grid-cols-2">
        <form className="rounded border p-4" onSubmit={onRequestExport}>
          ...
          <input
            ...
            placeholder="User ID"
            value={exportForm.subjectUserId}
            onChange={(e) => setExportForm((v) => ({ ...v, subjectUserId: e.target.value }))}
          />
          <select
            className="mb-3 w-full rounded border p-2"
            value={exportForm.format}
            onChange={(e) =>
              setExportForm((v) => ({ ...v, format: e.target.value as 'json' | 'csv' }))
            }
          >
        ...
        <form className="rounded border p-4" onSubmit={onRequestDeletion}>
          ...
          <input
            className="mb-2 w-full rounded border p-2"
            placeholder="User ID"
            value={deleteForm.subjectUserId}
            onChange={(e) => setDeleteForm((v) => ({ ...v, subjectUserId: e.target.value }))}
          />
          <select
            className="mb-3 w-full rounded border p-2"
            value={deleteForm.mode}
            onChange={(e) =>
              setDeleteForm((v) => ({ ...v, mode: e.target.value as 'anonymize' | 'delete' }))
            }
          >
      ...
      <section className="rounded border p-4">
        ...
        <button className="rounded bg-black px-3 py-2 text-white" onClick={onExportAuditCsv}>
          Export CSV
        </button>
      </section>
      ...
      <section className="rounded border p-4">
        ...
        <button className="rounded bg-black px-3 py-2 text-white" onClick={onGenerateReport}>
      ...
      <section className="rounded border p-4">
        ...
        <form className="grid grid-cols-1 gap-2 md:grid-cols-4" onSubmit={onRecordConsent}>
          <input
            ...
            onChange={(e) => setConsentForm((v) => ({ ...v, userId: e.target.value }))}
          />
          <select
            className="rounded border p-2"
            value={consentForm.consentType}
            onChange={(e) => setConsentForm((v) => ({ ...v, consentType: e.target.value }))}
          >
          ...
          <input
            ...
            onChange={(e) => setConsentForm((v) => ({ ...v, version: e.target.value }))}
          />
          <select
            className="rounded border p-2"
            value={String(consentForm.granted)}
            onChange={(e) => setConsentForm((v) => ({ ...v, granted: e.target.value === 'true' }))}
          >
