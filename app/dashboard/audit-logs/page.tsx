    ...
    <div className="space-y-6 p-6">
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Track security-sensitive actions, data changes, and system events.</p>
        </div>
        <button
          onClick={exportCsv}
        ...
        {!loading && !error && (
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                {logs.map((log) => {
                  const timestamp = log.timestamp ? format(parseISO(log.timestamp), "MMM dd, yyyy HH:mm:ss") : "-";
                  return (
                    <Fragment key={log.id}>
                      <tr key={log.id} className="border-t border-[var(--border-subtle)]">
                        ...
                        <td className="px-4 py-3 text-sm">
                          ...
                          <div className="text-xs text-[var(--text-muted)]">{log.userEmail || log.userId}</div>
                        </td>
                        <td className="px-4 py-3 text-sm">
                        ...
                        <td className="px-4 py-3 text-sm">
                          ...
                          {log.resourceId && <div className="text-xs text-[var(--text-muted)]">{log.resourceId}</div>}
                        </td>
                        <td className="px-4 py-3 text-sm">
                          <span
                          ...
                          <td className="px-4 py-4 text-sm text-[var(--text-muted)]" colSpan={6}>
                            <div className="grid gap-3 md:grid-cols-3">
                              {log.changes?.map((change) => (
                                <div key={`${log.id}-${change.field}`} className="rounded-xl border p-3">
                                  <div className="text-xs uppercase text-[var(--text-muted)]">{change.field}</div>
                                  <div className="mt-2 text-xs text-[var(--text-muted)]">Before</div>
                                  <div className="text-sm font-semibold">{formatValue(change.oldValue)}</div>
                                  <div className="mt-2 text-xs text-[var(--text-muted)]">After</div>
                                  <div className="text-sm font-semibold">{formatValue(change.newValue)}</div>
                                </div>
                              ))}
                            </div>
                            {log.errorMessage && (
                              <div className="mt-3 text-sm text-red-600">Error: {log.errorMessage}</div>
                            )}
                          </td>
                        </tr>
                      )}
