...
export default function ProductionManagerPerformancePage() {
  ...
  useEffect(() => {
    fetch("/api/admin/users/list", { credentials: "include" }).then((r) => r.json()).then(async (d) => {
      const users = (Array.isArray(d) ? d : d.users || []).filter((u: any) => u.role === "production");
      const next = await Promise.all(users.map(async (u: any) => {
        const [t, a] = await Promise.all([
          ...
          fetch(`/api/performance/actuals?userId=${u.uid}&period=${period}&periodType=${periodType}&role=production`, { credentials: "include" }).then((r) => r.json()),
        ]);
  ...
  return <div className="space-y-6"><div><h1 className="text-2xl font-semibold text-[var(--text-primary)]">My Performance</h1><p className="text-sm text-[var(--text-muted)] mt-1">Production workload and delivery status</p></div>
    ...
    <section className="card p-4"><h2 className="font-semibold mb-3">Team performance</h2><table className="w-full text-sm"><thead><tr><th className="text-left">Team Member</th><th>Jobs Completed</th><th>Jobs In Progress</th><th /></tr></thead><tbody>{rows.map((row) => <tr key={row.user.uid} className="border-t"><td className="py-3">{row.user.displayName || row.user.name || row.user.uid}</td><td><ProgressBar label="Jobs Completed" actual={Number(row.actuals.jobsCompleted || 0)} target={Number(row.metrics.jobsCompleted?.target || 0)} unit="count" size="sm" /></td><td><ProgressBar label="Jobs In Progress" actual={Number(row.actuals.jobsInProgress || 0)} target={Number(row.metrics.jobsInProgress?.target || 0)} unit="count" size="sm" /></td><td><button className="btn ghost">Set Targets</button></td></tr>)}</tbody></table></section>
