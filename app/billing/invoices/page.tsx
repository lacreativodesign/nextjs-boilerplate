...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`rounded-full px-2 py-0.5 text-xs font-semibold capitalize ${map[status] || "bg-[var(--surface-muted)] text-[var(--text-muted)]"}`}
    >
      {status}
    </span>
  );
  ...
  return (
    <main className="mx-auto max-w-4xl px-4 py-8">
      ...
      <div className="mb-6">
        ...
        <p className="page-subtitle">Your Bizosto subscription billing history.</p>
      </div>

      {loading && <div className="card p-6 text-sm text-[var(--text-muted)]">Loading invoices...</div>}
      {error && <div className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-700">{error}</div>}

      {!loading && !error && invoices.length === 0 && (
        <div className="card p-8 text-center text-sm text-[var(--text-muted)]">
          No invoices yet. Invoices appear after your first billing cycle.
        </div>
      )}
              ...
              {invoices.map((inv) => (
                <tr key={inv.id} className="border-t border-[var(--border-subtle)]">
                  ...
                  <td className="p-3 text-[var(--text-muted)]">
                    {inv.periodStart && inv.periodEnd ? `${fmt(inv.periodStart)} – ${fmt(inv.periodEnd)}` : "—"}
                  </td>
                  <td className="p-3 font-semibold text-[var(--text-primary)]">{fmtAmount(inv.amount, inv.currency)}</td>
                  <td className="p-3">
