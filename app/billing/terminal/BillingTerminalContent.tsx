...
export function BillingTerminalContent({ showShell = true }: { showShell?: boolean }) {
  ...
  const loadTerminal = async () => {
    try {
      setLoading(true);
      setError(null);
      const res = await fetch('/api/billing/terminal', {
        cache: 'no-store',
        credentials: 'include',
      });
      ...
      {loading ? (
        ...
        <section className="card mx-auto max-w-xl text-center">
          ...
          <p className="mt-2 text-sm text-[var(--text-muted)]">
            You haven&apos;t connected a Stripe account yet. Connect your account to start accepting
        ...
        <>
          <section className="kpis">
            <article className="card kpi-card">
              ...
              <p className="text-3xl font-bold text-[var(--chart-series-2)]">
                {formatMoney(data?.metrics?.totalRevenue || 0)}
              </p>
              <p className="helper-text">{data?.metrics?.transactionCount || 0} transactions</p>
            </article>
            <article className="card kpi-card">
              <p className="helper-text mb-1">Platform Fees Paid</p>
              <p className="text-3xl font-bold text-[var(--text-muted)]">
                {formatMoney(data?.metrics?.totalFees || 0)}
              </p>
            ...
            <article className="card kpi-card">
              ...
              <p className="text-3xl font-bold text-[var(--erp-blue)]">
                {formatMoney(data?.metrics?.netRevenue || 0)}
              </p>
              <p className="helper-text">After platform fee</p>
            </article>
            <article className="card kpi-card">
              <p className="helper-text mb-1">This Month</p>
              <p className="text-3xl font-bold text-[var(--chart-series-5)]">
                {formatMoney(data?.metrics?.thisMonthRevenue || 0)}
              </p>
                      ...
                      paginatedTransactions.map((transaction) => {
                        ...
                        return (
                          <tr key={transaction.id}>
                            ...
                            <td className="text-[var(--chart-series-2)]">
          ...
          <section className="card">
                    ...
                    {(data?.payouts || []).length === 0 ? (
                      ...
                      (data?.payouts || []).map((payout) => (
                        <tr key={payout.id}>
                          ...
                          <td>
                            <span
                              className={`rounded-full px-2 py-1 text-xs font-semibold ${statusClass(payout.status)}`}
                            >
                              {payout.status}
                            </span>
                          </td>
            ...
            <p className="mt-4 text-sm text-[var(--text-muted)]">
              Payout schedules are managed in your Stripe Dashboard.
            </p>
            <a
          ...
          {(data?.disputes || []).length > 0 && (
            <section className="card">
              ...
              <div className="mt-3 rounded-xl border border-[var(--danger)] bg-[var(--danger-soft)] p-3 text-sm text-[var(--danger)]">
                ⚠ You have {(data?.disputes || []).length} open dispute(s). Respond in your Stripe
                Dashboard.
              </div>
                      ...
                      {(data?.disputes || []).map((dispute) => {
                        ...
                        return (
                          <tr key={dispute.id}>
                            ...
                            <td className={urgentClass}>
                              {dispute.dueBy ? formatDate(dispute.dueBy) : '-'}
                            </td>
                            <td>
                              <a
              ...
              <p className="mt-4 text-sm text-[var(--text-muted)]">
                Bizosto does not manage disputes. You are the merchant of record and must handle
                disputes directly with Stripe.
              </p>
            </section>
          )}
          ...
          <section className="card">
            <p className="text-sm text-[var(--text-muted)]">
              Stripe Account:{' '}
              {data?.connectStatus?.businessName || data?.connectStatus?.email || '-'} | Status:
              Connected ✓ | Charges: {data?.connectStatus?.chargesEnabled ? 'enabled' : 'disabled'}{' '}
              | Payouts: {data?.connectStatus?.payoutsEnabled ? 'enabled' : 'disabled'}
            </p>
            <Link
