...
export async function POST(req: Request) {
  try {
    ...
    void recordIngestUsage({ tenantId, endpoint: 'ingest/orders', method: 'POST' });
    ...
    if (!stripeSessionId) {
      return NextResponse.json(
        { ok: false, error: 'stripeSessionId is required.' },
        { status: 400 },
      );
    }
    if (!customerEmail || !isValidEmail(customerEmail)) {
      return NextResponse.json(
        { ok: false, error: 'Valid customerEmail is required.' },
        { status: 400 },
      );
    }
    ...
    const projectType =
      requestedType && PROJECT_TYPES.includes(requestedType) ? requestedType : 'Other';
    ...
    if (existingClient) {
      ...
      clientName = String(
        existingClient.data.companyName || existingClient.data.name || companyName,
      );
          ...
          .set(
            {
              ...
              stripeSubscriptionId:
                stripeSubscriptionId || existingClient.data.stripeSubscriptionId || null,
              subscriptionStatus: isRecurring
                ? 'active'
                : existingClient.data.subscriptionStatus || null,
              updatedAt: now,
            },
            { merge: true },
          );
    ...
    const projectName =
      normalizeOptionalString(body.projectName) || packageName || `${projectType} project`;
    ...
    return NextResponse.json(
      {
        ...
        clientId,
        projectId,
        createdClient: createdNewClient,
