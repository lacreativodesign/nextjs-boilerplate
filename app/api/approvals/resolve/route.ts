  ...
  try {
    ...
    if (kind === "discount") {
      ...
      await adminDb.runTransaction(async (tx) => {
        ...
        const currentStatus = String(dealData.discountStatus || (dealData.discountPct > 0 ? "pending" : "none"));
      ...
      const notifyTargets = Array.from(
        new Set([dealData.ownerId, dealData.createdBy, dealData.discountRequestedByUid].filter(Boolean))
      ) as string[];
      ...
      await Promise.all(
        Array.from(notifyIds).map((uid) =>
          createNotification({
            ...
            entityType: "change_request",
            entityId: id,
            deepLink: project?.ownerAmUid ? "/am/change-requests" : "/admin/projects/change-requests",
            createdBy: { uid: me.uid, name: actorName },
            tenantId,
          })
        )
