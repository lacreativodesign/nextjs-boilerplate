...
function canRequest(type: ApprovalType, role: string) {
  ...
  if (type === 'production_override')
    return ['production', 'production_manager'].includes(normalized);
  return false;
}
...
export async function POST(req: Request) {
  try {
    ...
    const requestedData =
      typeof body?.requestedData === 'object' && body?.requestedData ? body.requestedData : {};
    ...
    if (type === 'change_request') {
      ...
      if (!changeRequestId) {
        return NextResponse.json(
          { ok: false, error: 'Missing change request id.' },
          { status: 400 },
        );
      }
      ...
      if (projectId && projectId !== entityId) {
        return NextResponse.json(
          { ok: false, error: 'Change request does not match project.' },
          { status: 400 },
        );
      }
    ...
    const before =
      type === 'discount'
        ? {
            discountPct: Number(entityData.discountPct || 0),
            finalPriceUsd: Number(entityData.finalPriceUsd || 0),
          }
        : type === 'change_request'
    ...
    if (requiredRole) {
      getUsersByRoles([requiredRole, 'admin'], tenantId)
        .then((approvers) => {
          const typeLabel =
            type === 'discount'
              ? 'Discount Approval'
