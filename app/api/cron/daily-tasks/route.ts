...
export async function GET(request: NextRequest) {
  try {
    ...
    const [cleanupSummary, complianceReports, dailyDigestSent, exchangeRateRefreshed, expiredSessionsRevoked, archivedProjects] =
      await Promise.all([
    ...
    const [scheduledReports, syncJobs] = await Promise.all([enqueueDailyReportJobs(), enqueueDailyIntegrationSyncJobs()]);
...
async function cleanupExpiredSessions() {
  const now = new Date();
  const snapshot = await adminDb.collection("sessions").where("active", "==", true).where("expiresAt", "<=", now).limit(500).get();
...
async function archiveCompletedProjectsOlderThan90Days() {
  ...
  const snapshot = await adminDb.collectionGroup("projects").where("status", "in", ["completed", "delivered"]).limit(500).get();
  if (snapshot.empty) return 0;
