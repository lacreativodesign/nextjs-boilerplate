...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!payload.success) {
      return NextResponse.json({ ok: false, error: "Invalid dependency payload.", details: payload.error.flatten() }, { status: 400 });
    }

    const { projectId, predecessorTaskId, successorTaskId, type, lagDays } = payload.data;
    if (predecessorTaskId === successorTaskId) {
      return NextResponse.json({ ok: false, error: "Task cannot depend on itself." }, { status: 400 });
    }
