...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (!validated.success) {
      return NextResponse.json({ ok: false, error: "Invalid reschedule payload.", details: validated.error.flatten() }, { status: 400 });
    }
    ...
    updates.set(params.id, { startDate: validated.data.startDate, dueDate: validated.data.endDate });
