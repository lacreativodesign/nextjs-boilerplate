  ...
  try {
    ...
    const created = await createDefect(
      ...
      },
      {
        ...
        testCaseIds: Array.isArray(body.testCaseIds)
          ? body.testCaseIds.map((id) => cleanString(id)).filter(Boolean)
          : [],
      },
    );
...
export async function GET(request: Request) {
  try {
    ...
    if (['critical', 'high', 'medium', 'low'].includes(severity))
      filters.severity = severity as DefectSeverity;
    if (['open', 'in_progress', 'resolved', 'closed', 'reopened'].includes(status))
      filters.status = status as DefectStatus;
