...
export async function PUT(request: Request, context: { params: { id: string } }) {
  try {
    ...
    if (
      body.status &&
      !['open', 'in_progress', 'resolved', 'closed', 'reopened'].includes(body.status)
    ) {
    ...
    const defect = await updateDefect(
      ...
      {
        uid: auth.user.uid,
        name: auth.user.name || auth.user.fullName || auth.user.displayName || auth.user.uid,
      },
      {
        status: body.status,
        severity: body.severity,
        assignedToUid:
          body.assignedToUid === undefined ? undefined : cleanString(body.assignedToUid) || null,
        rootCauseCategory:
          body.rootCauseCategory === undefined
            ? undefined
            : cleanString(body.rootCauseCategory) || null,
        rootCauseNotes:
          body.rootCauseNotes === undefined ? undefined : cleanString(body.rootCauseNotes) || null,
    ...
    if (error?.message === 'Defect not found')
