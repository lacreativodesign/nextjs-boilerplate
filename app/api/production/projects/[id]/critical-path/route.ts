  ...
  try {
    ...
    const tasks = tasksSnap.docs.map((doc) => {
      ...
      return {
        ...
        startDate,
        endDate,
        durationDays: data.estimatedHours
          ? Math.max(1, Math.ceil(Number(data.estimatedHours) / 8))
          : 1,
        ...
        assignedResources: Array.isArray(data.assignedResources)
          ...
          : data.assignedTo
            ? [
                {
                  resourceId: data.assignedTo,
                  resourceName: data.assignedToName || 'Unassigned',
    ...
    return NextResponse.json(
      { ok: false, error: 'Unable to calculate critical path.' },
      { status: 500 },
    );
  }
}
