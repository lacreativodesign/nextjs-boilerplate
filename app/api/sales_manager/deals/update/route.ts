...
export async function POST(req: Request) {
  try {
    ...
    await adminDb.runTransaction(async (tx) => {
      ...
      finalPriceUsd = parseNumber(
        data.finalPriceUsd,
        listPriceUsd - (listPriceUsd * discountPct) / 100,
      );
      ...
      const discountAction =
        actionRaw ||
        (payload.discountApproved === true
          ? 'approve'
          : payload.discountApproved === false
            ? 'reject'
            : '');
      if (discountAction) {
        const currentStatus = parseString(
          data.discountStatus,
          discountPct > 0 ? 'pending' : 'none',
        );
    ...
    const stageTitle = stageChanged
      ? isClosed
        ...
        : 'Deal stage moved'
      : 'Deal updated';
    const stageDescription = stageChanged
      ? `Deal ${id} moved to ${nextStage}`
      : `Deal ${id} updated`;
    ...
    if (discountDecision) {
      ...
      if (!approvalSnap.empty) {
        ...
        const chainIndex = approvalChain.findIndex(
          (item) => item?.role === 'sales_manager' && !item?.decision,
        );
        if (chainIndex >= 0) {
      ...
      const notifyTargets = Array.from(
        new Set([ownerId, createdByUid].filter(Boolean)),
