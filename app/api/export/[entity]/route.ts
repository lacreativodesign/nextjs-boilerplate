...
const schema = z.object({
  ...
  filters: z.array(z.object({ field: z.string().min(1), operator: z.literal("eq"), value: z.union([z.string(), z.number(), z.boolean()]) })).optional(),
  pageSize: z.number().int().min(1).max(500).optional(),
  pageCursor: z.string().optional(),
});

const entitySchema = z.enum(["clients", "invoices", "products", "employees", "projects", "tasks", "time_entries"]);
