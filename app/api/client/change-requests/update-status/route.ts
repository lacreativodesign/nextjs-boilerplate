  ...
  try {
    ...
    if (!changeRequestId || !['approve', 'reject', 'approved', 'rejected'].includes(decision)) {
      return NextResponse.json(
        { ok: false, error: 'Invalid change request decision.' },
        { status: 400 },
      );
    }
    ...
    const docTenant = String(data.tenantId || '');
    ...
    if (['Approved', 'Rejected', 'Completed'].includes(currentStatus)) {
      return NextResponse.json(
        { ok: false, error: 'Change request already finalized.' },
        { status: 400 },
      );
    }
    ...
    await logEvent({
      ...
      type:
        toStatus === 'Approved'
          ? 'change_request.client_approved'
          : 'change_request.client_rejected',
    ...
    return NextResponse.json(
      { ok: false, error: 'Unable to update change request.' },
      { status: 500 },
    );
  }
}
