...
const channelSchema = z.string().refine((value): value is (typeof USER_NOTIFICATION_CHANNELS)[number] => {
  return USER_NOTIFICATION_CHANNELS.includes(value as (typeof USER_NOTIFICATION_CHANNELS)[number]);
});
const eventTypeSchema = z.string().refine((value) => {
  return USER_NOTIFICATION_EVENT_TYPES.includes(value as (typeof USER_NOTIFICATION_EVENT_TYPES)[number]);
});
