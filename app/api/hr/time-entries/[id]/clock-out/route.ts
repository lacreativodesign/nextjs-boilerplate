...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const entry = await TimeTrackingService.clockOut(id, me.tenantId as string, me.uid, parsed.data.breakMinutes || 0);

    return NextResponse.json({ ok: true, entry });
  } catch (err) {
    console.error("HR clock-out error", err);
    return NextResponse.json({ ok: false, error: err instanceof Error ? err.message : "Failed to clock out" }, { status: 400 });
  }
}
