...
export async function POST(request: NextRequest) {
  try {
    ...
    getUsersByRoles(['hr', 'admin'], me.tenantId)
      .then((hrTeam) => {
        const startFormatted = new Date(payload.startDate).toLocaleDateString('en-US', {
          year: 'numeric',
          month: 'long',
          day: 'numeric',
        });
        const endFormatted = new Date(payload.endDate).toLocaleDateString('en-US', {
          year: 'numeric',
          month: 'long',
          day: 'numeric',
        });
...
<tr><td style="background:#F1F5F9;padding:20px 32px;border-top:1px solid #E2E8F0;"><p style="margin:0;font-size:12px;color:#94A3B8;text-align:center;">© ${new Date().getFullYear()} Bizosto · <a href="https://bizosto.com" style="color:#012167;text-decoration:none;">bizosto.com</a></p></td></tr>
    ...
    return NextResponse.json(
      { ok: false, error: err instanceof Error ? err.message : 'Failed to submit leave request' },
      { status: 400 },
    );
  }
}
