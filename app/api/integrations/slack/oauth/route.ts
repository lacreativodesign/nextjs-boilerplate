  ...
  try {
    ...
    if (body?.start) {
      const state = await createSlackOAuthState({
        ...
        returnTo: typeof body.returnTo === "string" ? body.returnTo : "/admin/settings/integrations",
      });
...
export async function GET(request: Request) {
  ...
  if (error) {
    return NextResponse.redirect(new URL(`/admin/settings/integrations?slack_error=${encodeURIComponent(error)}`, request.url));
  }

  if (!code || !state) {
    return NextResponse.redirect(new URL("/admin/settings/integrations?slack_error=missing_code_or_state", request.url));
  }
