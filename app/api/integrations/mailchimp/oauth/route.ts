  ...
  try {
    ...
    if (!code || !state) {
      return NextResponse.json(
        { ok: false, error: 'Either start, apiKey, or code/state is required.' },
        { status: 400 },
      );
    }
    ...
    return NextResponse.json(
      { ok: false, error: error?.message || 'Mailchimp OAuth failed.' },
      { status: 500 },
    );
  }
}
...
export async function GET(request: Request) {
  ...
  if (error) {
    return NextResponse.redirect(
      new URL(
        `/admin/settings/integrations/mailchimp?mailchimp_error=${encodeURIComponent(error)}`,
        request.url,
      ),
    );
  }
  ...
  if (!code || !state) {
    return NextResponse.redirect(
      new URL(
        '/admin/settings/integrations/mailchimp?mailchimp_error=missing_code_or_state',
        request.url,
      ),
    );
  }
  ...
  try {
    ...
    return NextResponse.redirect(
      new URL(
        `/admin/settings/integrations/mailchimp?mailchimp_error=${encodeURIComponent(oauthError?.message || 'oauth_failed')}`,
        request.url,
      ),
    );
  }
