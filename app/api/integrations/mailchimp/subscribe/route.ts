  ...
  try {
    ...
    if (!email || !firstName || !lastName) {
      return NextResponse.json(
        { ok: false, error: 'email, firstName, and lastName are required.' },
        { status: 400 },
      );
    }
    ...
    const result = await subscribeSingleContact({
      ...
      customer: {
        ...
        type:
          body.type === 'prospect' || body.type === 'customer' || body.type === 'partner'
            ? body.type
            : 'lead',
    ...
    return NextResponse.json(
      { ok: false, error: error?.message || 'Unable to subscribe contact.' },
      { status: 500 },
    );
  }
}
