...
export async function GET() {
  try {
    ...
    return NextResponse.json(
      { ok: false, error: error?.message || 'Unable to list Mailchimp audiences.' },
      { status: 500 },
    );
  }
}
...
export async function PUT(request: Request) {
  try {
    ...
    await updateMailchimpSettings(auth.user.tenantId, auth.user.uid, {
      ...
      defaultAudienceId:
        body.defaultAudienceId === null || typeof body.defaultAudienceId === 'string'
          ? body.defaultAudienceId
          : undefined,
      tagMapping:
    ...
    return NextResponse.json(
      { ok: false, error: error?.message || 'Unable to update Mailchimp settings.' },
      { status: 500 },
    );
  }
}
