  ...
  try {
    ...
    if (action === 'upload') {
      if (!body.fileName || !body.base64Content) {
        return NextResponse.json(
          { ok: false, error: 'fileName and base64Content are required.' },
          { status: 400 },
        );
      }
    ...
    if (action === 'sync_folder') {
      ...
      const mapping = await ensureOneDriveFolderMapping({
        ...
        parentOneDriveFolderId: body.parentOneDriveFolderId
          ? String(body.parentOneDriveFolderId)
          : undefined,
      });
