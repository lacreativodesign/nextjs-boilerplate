...
export async function POST(request: Request) {
  try {
      ...
      const state = await createDocusignOAuthState({
        ...
        returnTo:
          typeof body.returnTo === 'string'
            ? body.returnTo
            : '/admin/settings/integrations/docusign',
      });
    ...
    return NextResponse.json(
      { ok: false, error: error?.message || 'DocuSign OAuth failed.' },
      { status: 500 },
    );
  }
}
  ...
  if (!code || !state) {
    if (error) {
      return NextResponse.redirect(
        new URL(
          `/admin/settings/integrations/docusign?docusign_error=${encodeURIComponent(error)}`,
          request.url,
        ),
      );
    }
    ...
    try {
      ...
      return NextResponse.json(
        { ok: false, error: readError?.message || 'Unable to fetch DocuSign state.' },
        { status: 500 },
      );
    }
  }
