  ...
  try {
    ...
    if (contentType.includes("application/json")) {
      ...
      const signerEmails = Array.isArray(body.signerEmails)
        ? body.signerEmails.map((item: unknown) => String(item || "").trim().toLowerCase()).filter(Boolean)
        : [];
    ...
    const message = String(formData.get("message") || "Please review and sign this document.").trim();

    if (!file) return NextResponse.json({ ok: false, error: "A file is required." }, { status: 400 });
    if (file.type !== "application/pdf") {
      return NextResponse.json({ ok: false, error: "Only PDF files are supported for DocuSign." }, { status: 400 });
    }
    if (!signerEmails.length) {
      return NextResponse.json({ ok: false, error: "At least one signer email is required." }, { status: 400 });
    }
