  ...
  try {
    ...
    if (!code || !state) {
      return NextResponse.redirect(
        new URL('/admin/settings/integrations/xero?error=Missing+OAuth+parameters', request.url),
      );
    }
    ...
    const selected =
      connections.find((item) => getString(item?.tenantId) === preferredOrgId) || connections[0];
