...
export async function POST(req: Request) {
  try {
    ...
    const nextOwnerAmUid =
      body?.ownerAmUid !== undefined
        ? cleanString(body.ownerAmUid) || null
        : data.ownerAmUid || null;
    const nextProductionUid =
      body?.productionUid !== undefined
        ? cleanString(body.productionUid) || null
        : data.productionUid || null;
    const assignmentChanged =
      nextOwnerAmUid !== (data.ownerAmUid || null) ||
      nextProductionUid !== (data.productionUid || null);
    ...
    if (body?.productionUid !== undefined) {
      ...
      updateData.productionName = nextProductionUid
        ? await resolveUserName(nextProductionUid)
        : null;
    }
