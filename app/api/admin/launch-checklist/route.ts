...
function calculateReadinessScore(params: {
  ...
  const totalChecks =
    params.automated.length +
    getAllManualCheckIds().length +
    getAllProductionConfigurationIds().length;
  ...
  const completedConfiguration = getAllProductionConfigurationIds().filter(
    (id) => params.productionConfiguration[id],
  ).length;
...
async function runAutomatedChecks(
  req: Request,
  storedLighthouseScore?: number | null,
): Promise<AutomatedCheckStatus[]> {
  ...
  try {
    ...
    indexDetails =
      error instanceof Error ? error.message : 'Unable to read Firestore index configuration.';
  }
  ...
  const lighthouseScore =
    typeof storedLighthouseScore === 'number'
      ...
      : Number(
          process.env.LIGHTHOUSE_PERFORMANCE_SCORE ||
            process.env.NEXT_PUBLIC_LIGHTHOUSE_PERFORMANCE_SCORE ||
            0,
        );
  ...
  return [
    ...
    },
    {
      ...
      details: endpointResults
        .map((entry) => `${entry.path} (${entry.status || 'network error'})`)
        .join(', '),
    },
    {
...
async function getLaunchMetrics(tenantId: string) {
  ...
  const conversionRate =
    activeTrials + paid === 0 ? 0 : Math.round((paid / (activeTrials + paid)) * 100);
...
export async function GET(req: Request) {
  try {
    ...
    return NextResponse.json(
      { ok: false, error: 'Unable to load launch checklist.' },
      { status: 500 },
    );
  }
}
  ...
  try {
    ...
    if (body.action === 'toggle-manual') {
      ...
      if (!getAllManualCheckIds().includes(id)) {
        return NextResponse.json(
          { ok: false, error: 'Invalid manual checklist item.' },
          { status: 400 },
        );
      }
      await docRef.set(
    ...
    if (body.action === 'toggle-production-config') {
      ...
      if (!getAllProductionConfigurationIds().includes(id)) {
        return NextResponse.json(
          { ok: false, error: 'Invalid production configuration item.' },
          { status: 400 },
        );
      }
      await docRef.set(
        {
          productionConfiguration: {
            ...stored.productionConfiguration,
            [id]: Boolean(body.checked),
          },
          updatedAt: new Date().toISOString(),
          updatedBy: auth.user.uid,
        },
    ...
    if (body.action === 'set-lighthouse-score') {
      ...
      if (!Number.isFinite(score) || score < 0 || score > 100) {
        return NextResponse.json(
          { ok: false, error: 'Lighthouse score must be a number between 0 and 100.' },
          { status: 400 },
        );
      }
      await docRef.set(
    ...
    if (body.action === 'send-launch-announcement') {
      ...
      if (recipients.length === 0) {
        return NextResponse.json(
          { ok: false, error: 'No admin recipients found for launch announcement.' },
          { status: 400 },
        );
      }
    ...
    return NextResponse.json(
      { ok: false, error: 'Unable to process launch checklist action.' },
      { status: 500 },
    );
  }
}
