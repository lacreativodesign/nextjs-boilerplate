...
async function generateNextInvoiceId(tenantId: string): Promise<string> {
  ...
  const ref = adminDb.collection('tenants').doc(tenantId).collection('counters').doc('invoices');
  ...
  try {
    ...
    if (!auth.user.tenantId) {
      return NextResponse.json(
        { ok: false, error: 'Tenant context is required.' },
        { status: 400 },
      );
    }
    ...
    if (lineItems.length === 0) {
      return NextResponse.json(
        { ok: false, error: 'Add at least one line item.' },
        { status: 400 },
      );
    }
    ...
    if (!isTaxExempt) {
      ...
      const taxRates = taxRatesSnap.docs.map((doc) => {
        ...
        return {
          ...
          applies_to:
            data.applies_to === 'subtotal_plus_other_taxes'
              ? 'subtotal_plus_other_taxes'
              : 'subtotal',
    ...
    const exchangeRate =
      currencyCode !== baseCurrency ? currencyConverter.getRate(currencyCode, baseCurrency) : 1;
