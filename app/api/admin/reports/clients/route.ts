  ...
  try {
    ...
    const segmentCoverage = [
      {
        ...
        value: filteredClients.filter((client) => Array.isArray(client.segmentServices) && client.segmentServices.length).length,
      },
      {
    ...
    const segmentOptions = {
      service: Array.from(
        new Set(
          filteredClients
            .flatMap((client) => (Array.isArray(client.segmentServices) ? client.segmentServices : []))
    ...
    return NextResponse.json({
      ok: true,
      keyAccounts,
      clientHealthDistribution: Array.from(clientHealthDistribution.entries()).map(([label, value]) => ({
        label,
        value,
      })),
      changeRequestsByClient: Array.from(changeRequestsByClient.values()).sort((a, b) => b.count - a.count).slice(0, 10),
      segmentCoverage,
      clients: clientRows,
      segmentOptions,
    });
    ...
    const safeMessage = isIndexError ? "Missing Firestore index." : "Unable to load client reports.";
