...
export async function getReportSettings() {
  ...
  return {
    arAgingBucketsDays: buckets
      .map((value: any) => parseNumber(value, 0))
      .filter((value: number) => value > 0),
    keyAccountUsdThreshold: DEFAULT_REPORT_SETTINGS.keyAccountUsdThreshold,
    atRiskAfterDays: parseNumber(
      workflowSettings.atRiskAfterDays,
      DEFAULT_REPORT_SETTINGS.atRiskAfterDays,
    ),
    overdueAfterDays: parseNumber(
      workflowSettings.overdueAfterDays,
      DEFAULT_REPORT_SETTINGS.overdueAfterDays,
    ),
    stageSlaDays: { ...DEFAULT_REPORT_SETTINGS.stageSlaDays, ...stageSlaDays },
    projectStages:
      workflowSettings.projectStages || Object.keys(DEFAULT_REPORT_SETTINGS.stageSlaDays),
  };
}
