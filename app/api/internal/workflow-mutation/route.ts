...
export async function POST(req: NextRequest) {
  ...
  if (!action || !entity || !tenantId) {
    return NextResponse.json(
      { ok: false, error: 'Missing required fields: action, entity, tenantId' },
      { status: 400 },
    );
  }
    ...
    switch (action) {
      case 'create_record': {
        if (!payload || typeof payload !== 'object') {
          return NextResponse.json(
            { ok: false, error: 'Missing payload for create_record' },
            { status: 400 },
          );
        }
      ...
      case 'update_field': {
        if (!recordId || !field) {
          return NextResponse.json(
            { ok: false, error: 'Missing recordId or field for update_field' },
            { status: 400 },
          );
        }
        ...
        if (normalizeTenantId(snapTenantId) !== normalizeTenantId(tenantId)) {
          return NextResponse.json(
            { ok: false, error: 'Tenant isolation violation' },
            { status: 403 },
          );
        }
        await docRef.set(
          { [String(field)]: value, updatedAt: new Date().toISOString() },
          { merge: true },
        );
        await writeAuditLog({
          tenantId,
      ...
      case 'delete_record': {
        if (!recordId) {
          return NextResponse.json(
            { ok: false, error: 'Missing recordId for delete_record' },
            { status: 400 },
          );
        }
