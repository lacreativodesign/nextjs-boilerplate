...
export async function GET(req: NextRequest) {
  try {
    ...
    const checks = [
      ...
      runWithTimeout({ id: "required-collections", name: "Required Firestore Collections" }, async () => {
        const requiredCollections = ["tenants", "users", "clients", "invoices", "projects", "notifications"];
        const snapshots = await Promise.all(
          requiredCollections.map((collection) => adminDb.collection(collection).limit(1).get())
        );

        const missingCollections = requiredCollections.filter((_, index) => snapshots[index].empty);
