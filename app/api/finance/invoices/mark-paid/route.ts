...
export async function POST(req: Request) {
  try {
    ...
    let invoiceSnapshot: FirebaseFirestore.DocumentSnapshot;
    let paymentRef: FirebaseFirestore.DocumentReference;
    let invoiceData: Record<string, any>;

    await adminDb.runTransaction(async (tx) => {
      invoiceSnapshot = await tx.get(invoiceRef);
      if (!invoiceSnapshot.exists) {
        throw new Error("Invoice not found.");
      }
      invoiceData = invoiceSnapshot.data() || {};
      ...
      const invoiceNumber = String(invoiceData.invoiceNumber || invoiceData.orderId || id);
      tx.set(paymentRef, {
        id: paymentRef.id,
    ...
    const invoiceNumber = String(invoiceData.invoiceNumber || invoiceData.orderId || id);
    const clientName = String(invoiceData.clientName || "");
    ...
    await Promise.all(
      financeIds.map((uid) =>
        createNotification({
          toUserId: uid,
          title: "Invoice paid",
          body: `Invoice ${invoiceNumber} marked paid.`,
          type: "success",
          entityType: "invoice",
          entityId: id,
    ...
    await logEvent({
      ...
      metadata: {
        ...
        paymentAmount: Number(invoiceData.totalAmount ?? invoiceData.amountTotalUsd ?? 0),
      },
    });
    ...
    return NextResponse.json({ ok: true, paymentId: paymentRef?.id || null });
  } catch (err: any) {
    console.error("finance/invoices mark-paid error:", err);
    const message = String(err?.message || "Unable to mark invoice paid.");
