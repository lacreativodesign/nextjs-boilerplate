...
export async function GET(_: Request, { params }: { params: { taxRateId: string } }) {
  try {
    ...
    if (!snap.exists) return NextResponse.json({ ok: false, error: "Tax rate not found" }, { status: 404 });
  ...
  try {
    ...
    if (body.rate !== undefined) {
      ...
      if (!Number.isFinite(rate) || rate < 0 || rate >= 100) {
        return NextResponse.json({ ok: false, error: "rate must be between 0 and 100" }, { status: 400 });
      }
      updates.rate = parseFloat(rate.toFixed(2));
    }

    if (body.isDefault === true) {
      const existingDefaults = await adminDb.collection("tax_rates").where("tenantId", "==", tenantId).get();
