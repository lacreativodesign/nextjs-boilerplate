...
export async function POST(req: Request) {
  try {
    ...
    const companyName = parseString(
      body.companyName || body.company,
      existing.companyName || existing.company || '',
    );
    const contactName = parseString(
      body.contactName || body.name,
      existing.contactName || existing.name || '',
    );
    const contactEmail = parseString(
      body.contactEmail || body.email,
      existing.contactEmail || existing.email || '',
    );
    const contactPhone = parseString(
      body.contactPhone || body.phone,
      existing.contactPhone || existing.phone || '',
    );
    ...
    const expectedValueUsd = parseNumber(
      body.expectedValueUsd,
      Number(existing.expectedValueUsd || 0),
    );
    ...
    const interestedServices = Array.isArray(body.interestedServices)
      ...
      : Array.isArray(existing.interestedServices)
        ? existing.interestedServices
            .map((item: any) => parseString(item, '').trim())
            .filter(Boolean)
        : [];
    const probability = parseNumber(body.probability, Number(existing.probability || 0));
    const rawStatusInput = parseString(
      body.status || body.stage,
      existing.status || 'new',
    ).toLowerCase();
    ...
    const assignmentChanged =
      String(existing.ownerUid || existing.ownerId || '') !== String(ownerId || '');
