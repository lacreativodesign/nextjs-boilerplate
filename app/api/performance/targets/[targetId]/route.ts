...
const managerRoles = new Set(["admin", "super_admin", "sales_manager", "am_manager", "production_manager", "hr", "finance"]);
...
export async function PATCH(req: NextRequest, { params }: { params: { targetId: string } }) {
  ...
  if (currentData.tenantId !== me.tenantId) return NextResponse.json({ ok: false, error: "Forbidden" }, { status: 403 });

  const body = (await req.json().catch(() => null)) as { metrics?: Record<string, unknown>; period?: string } | null;
...
export async function DELETE(_: NextRequest, { params }: { params: { targetId: string } }) {
  ...
  const current = await targetRef.get();
  if (!current.exists) return NextResponse.json({ ok: false, error: "Not found" }, { status: 404 });
  if (current.data()?.tenantId !== me.tenantId) return NextResponse.json({ ok: false, error: "Forbidden" }, { status: 403 });
