...
async function safeCount(
  queryBuilder: () => Promise<FirebaseFirestore.QuerySnapshot>,
): Promise<number> {
  try {
...
export async function GET(req: NextRequest) {
  ...
  if (!period)
    return NextResponse.json({ ok: false, error: 'period is required' }, { status: 400 });
  ...
  if (requestedRole === 'sales') {
    ...
    actuals.revenueGenerated = assignedWins.reduce(
      (sum, doc) => sum + Number(doc.data()?.valueUsd || doc.data()?.finalPriceUsd || 0),
      0,
    );
  } else if (requestedRole === 'sales_manager') {
    const users = await safeDocs(() =>
      adminDb
        .collection('users')
        .where('tenantId', '==', tenantId)
        .where('role', '==', 'sales')
    ...
    actuals.leadsCreated = leadsDocs.filter((doc) =>
      ids.has(String(doc.data()?.createdBy || '')),
    ).length;
    const teamWins = winsDocs.filter((doc) =>
      ids.has(String(doc.data()?.closedBy || doc.data()?.assignedTo || '')),
    );
    actuals.dealsClosed = teamWins.length;
    actuals.revenueGenerated = teamWins.reduce(
      (sum, doc) => sum + Number(doc.data()?.valueUsd || doc.data()?.finalPriceUsd || 0),
      0,
    );
  } else if (requestedRole === 'am' || requestedRole === 'am_manager') {
    actuals.activeClients = await safeCount(() =>
      adminDb
        .collection('clients')
        .where('tenantId', '==', tenantId)
    ...
    actuals.revenueInvoiced = invoices.reduce(
      (sum, doc) => sum + Number(doc.data()?.amount || doc.data()?.total || 0),
      0,
    );
  }
