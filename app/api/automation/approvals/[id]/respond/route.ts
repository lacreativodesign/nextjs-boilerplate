...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  ...
  if (data.status !== 'pending') {
    return NextResponse.json(
      { ok: false, error: 'Approval request already resolved.' },
      { status: 400 },
    );
  }
  ...
  const currentStepIndex = Number.isInteger(data.currentStepIndex)
  ...
  if (!touched) {
    return NextResponse.json(
      { ok: false, error: 'User is not assigned to this approval step.' },
      { status: 403 },
    );
  }
  ...
  const nextIndex =
    mode === 'sequential' ? Math.min(currentStepIndex + 1, nextSteps.length - 1) : currentStepIndex;
