...
export async function POST(_req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    if (data.tenantId !== auth.user.tenantId)
      return NextResponse.json({ ok: false, error: 'Forbidden' }, { status: 403 });

    const firstEvent =
      Array.isArray(data.events) && data.events.length > 0 ? data.events[0] : 'invoice.created';
    ...
    return NextResponse.json(
      { ok: false, error: 'Unable to send test webhook event.' },
      { status: 500 },
    );
  }
}
