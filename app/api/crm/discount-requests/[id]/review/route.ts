...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    if (!canApproveDiscount(auth.user.role)) {
      throw new AppError({ message: "Only sales manager can review requests.", code: "FORBIDDEN", status: 403 });
    }

    const payload = await req.json();
    const decision = String(payload.decision || "");
    if (!["approved", "rejected"].includes(decision)) {
      throw new AppError({ message: "decision must be approved or rejected.", code: "VALIDATION_ERROR", status: 400 });
    }

    const reqRef = adminDb.collection("discountRequests").doc(params.id);
    await adminDb.runTransaction(async (tx) => {
      const reqSnap = await tx.get(reqRef);
      if (!reqSnap.exists) throw new AppError({ message: "Request not found.", code: "NOT_FOUND", status: 404 });
