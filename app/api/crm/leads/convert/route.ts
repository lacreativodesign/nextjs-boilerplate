...
export async function POST(req: Request) {
  try {
    ...
    if (!canCreateLeads(auth.user.role)) {
      throw new AppError({ message: "Only sales can convert leads.", code: "FORBIDDEN", status: 403 });
    }
    ...
    await adminDb.runTransaction(async (tx) => {
      ...
      if (!leadSnap.exists) throw new AppError({ message: "Lead not found.", code: "NOT_FOUND", status: 404 });
