...
export async function POST(req: NextRequest) {
  try {
    ...
    if (
      ...
      ['Unauthorized', 'Session expired', 'User not found'].includes(error.message)
    ) {
    ...
    if (error instanceof Error && error.message === 'TENANT_ALREADY_EXISTS') {
      return NextResponse.json(
        { success: false, error: 'Tenant already exists.' },
        { status: 409 },
      );
    }
    ...
    return NextResponse.json(
      { success: false, error: 'Unable to create tenant workspace.' },
      { status: 500 },
    );
  }
}
