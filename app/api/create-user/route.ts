...
export async function POST(req: Request) {
  ...
  try {
    ...
    // Atomic staff-seat reservation, held until the identity is committed. This legacy
    // surface enforces exactly the same ceiling as app/api/admin/users/create, and
    // cannot be used by two concurrent callers to overshoot it.
    const seat = await reserveStaffSeat(tenantId, targetRole, 'legacy_create');
    if (!seat.ok) {
