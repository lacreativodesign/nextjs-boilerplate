...
export default function AMChangeRequestsPage() {
  ...
  const headerLabel = (label: string) => (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
      <span>{label}</span>
      <span style={{ width: 14, display: 'inline-block', textAlign: 'center', opacity: 0.35 }}>
        •
      </span>
    </span>
  );
  ...
  const filtered = useMemo(() => {
    ...
    return changeRequests.filter((cr) => {
      ...
      if (q) {
        const hay = [cr.projectName, cr.clientName, cr.title]
          .filter(Boolean)
          .join(' ')
          .toLowerCase();
  ...
  const handleStatusUpdate = async (toStatus: string) => {
    ...
    try {
      ...
      setChangeRequests((prev) =>
        prev.map((item) => (item.id === selected.id ? { ...item, status: toStatus } : item)),
      );
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const requestApproval = async () => {
    ...
    try {
      ...
      setChangeRequests((prev) =>
        prev.map((item) =>
          item.id === selected.id ? { ...item, approvalStatus: 'pending' } : item,
        ),
      );
    } catch (err) {
      console.error(err);
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="page-subtitle">
          Coordinate scope updates and execution progress for your projects.
        </p>
      </div>
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <MasterSelect
            ...
            options={[
              ...
              ...CHANGE_REQUEST_STATUSES.map((value) => ({ value, label: value })),
            ]}
          />
          <MasterSelect
            value={typeFilter}
            ...
            options={[
              ...
              ...CHANGE_REQUEST_TYPES.map((value) => ({ value, label: value })),
            ]}
          />
          <MasterSelect
            ...
            options={[
              ...
              ...CHANGE_REQUEST_PRIORITIES.map((value) => ({ value, label: value })),
            ]}
          />
        </div>
      </div>
            ...
            <tbody>
              {filtered.map((cr) => (
                <tr key={cr.id}>
                  ...
                  <td style={{ ...cellStyle, textAlign: 'center' }}>
                    {fmtDate(cr.updatedAt || cr.createdAt)}
                  </td>
                  <td style={{ ...cellStyle, textAlign: 'center' }}>
...
function KpiCard({ label, value }: { label: string; value: number }) {
  return (
    <div className="card kpi-card" style={{ padding: 14, borderRadius: 16 }}>
      <div
        style={{ fontSize: 12, textTransform: 'uppercase', letterSpacing: '0.08em', opacity: 0.6 }}
      >
        {label}
      </div>
