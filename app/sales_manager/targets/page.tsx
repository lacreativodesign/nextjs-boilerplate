...
export default function SalesManagerTargetsPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const res = await fetch(`/api/sales_manager/targets?month=${encodeURIComponent(month)}`, {
          cache: 'no-store',
        });
      ...
      <div className="card" style={{ padding: 16, borderRadius: 16 }}>
        {loading ? (
          ...
          <div style={{ display: 'grid', gap: 12 }}>
            {targets.map((row) => {
              const progress = row.targetUsd
                ? Math.min((row.achievedUsd / row.targetUsd) * 100, 100)
                : 0;
