...
export default function SalesManagerPipelinePage() {
  ...
  const reps = useMemo(() => ["all", ...Array.from(new Set(deals.map((d) => d.assignedSalesId).filter(Boolean)))], [deals]);
          ...
          <tbody>
            {deals.map((deal) => (
              <tr key={deal.id} className="border-t">
                <td>{deal.title}</td><td>{deal.stage}</td><td>${deal.valueUSD.toLocaleString()}</td><td>{deal.assignedSalesId}</td>
                <td><Link className="btn" href={`/sales_manager/deals/${deal.id}`}>Open</Link></td>
              </tr>
            ))}
          </tbody>
        </table>
