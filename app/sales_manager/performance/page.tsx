...
export default function SalesManagerPerformancePage() {
  ...
  useEffect(() => {
    ...
    apiFetch('/api/admin/users/list')
      .then((r) => r.json())
      .then(async (list) => {
        const users = (Array.isArray(list) ? list : list.users || []).filter(
          (u: any) => u.role === 'sales',
        );
        if (!active) return;
        setTeamUsers(users);
        const rows = await Promise.all(
          users.map(async (u: TeamUser) => {
            const [t, a] = await Promise.all([
              ...
              apiFetch(
                `/api/performance/actuals?userId=${u.uid}&period=${period}&periodType=${periodType}&role=sales`,
              ).then((r) => r.json()),
            ]);
            ...
            const overall = percentages.length
              ? percentages.reduce((s, n) => s + n, 0) / percentages.length
              : 0;
  ...
  const aggregate = useMemo(() => {
    ...
    const totalRevenue = teamRows.reduce(
      (sum, row) => sum + Number(row.actuals.revenueGenerated || 0),
      0,
    );
    const avgAchievement =
      teamRows.reduce((sum, row) => sum + Number(row.overall || 0), 0) / teamRows.length;
    return {
      totalRevenue,
      avgAchievement,
      top: teamRows[0]?.user?.displayName || teamRows[0]?.user?.name || '—',
      bottom:
        teamRows[teamRows.length - 1]?.user?.displayName ||
        teamRows[teamRows.length - 1]?.user?.name ||
        '—',
    };
  }, [teamRows]);
    ...
    await apiFetch('/api/performance/targets', {
      ...
      body: JSON.stringify({
        ...
        metrics: {
          leadsCreated: {
            label: 'Leads Created',
            target: Number(form.leadsCreated || 0),
            unit: 'count',
          },
          dealsClosed: {
            label: 'Deals Closed',
            target: Number(form.dealsClosed || 0),
            unit: 'count',
          },
          revenueGenerated: {
            label: 'Revenue Generated',
            target: Number(form.revenueGenerated || 0),
            unit: 'USD',
          },
        },
      }),
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="page-subtitle mt-1">Sales metrics and pipeline health</p>
      </div>
      ...
      <section className="space-y-4">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
          <div className="card p-4">
            <p className="text-xs text-[var(--text-muted)]">Team Total Revenue</p>
            <p className="text-xl font-bold">{fmtCurrency(aggregate.totalRevenue)}</p>
          </div>
          <div className="card p-4">
            <p className="text-xs text-[var(--text-muted)]">Team Average Achievement</p>
            <p className="text-xl font-bold">{aggregate.avgAchievement.toFixed(1)}%</p>
          </div>
          <div className="card p-4">
            <p className="text-xs text-[var(--text-muted)]">Top Performer</p>
            <p className="text-xl font-bold">{aggregate.top}</p>
          </div>
          <div className="card p-4">
            <p className="text-xs text-[var(--text-muted)]">Needs Attention</p>
            <p className="text-xl font-bold">{aggregate.bottom}</p>
          </div>
        </div>
          ...
          <div className="drawer-panel drawer-panel--sm" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              <input
                ...
                value={form.leadsCreated}
                onChange={(e) => setForm((p) => ({ ...p, leadsCreated: e.target.value }))}
              />
              <input
                className="input w-full"
                type="number"
                ...
                onChange={(e) => setForm((p) => ({ ...p, dealsClosed: e.target.value }))}
              />
              <input
                className="input w-full"
                type="number"
                ...
                onChange={(e) => setForm((p) => ({ ...p, revenueGenerated: e.target.value }))}
              />
              <button className="btn w-full" onClick={saveTargets}>
                Save
              </button>
            </div>
          </div>
