        ...
        const next = await Promise.all(
          users.map(async (u: any) => {
            const [t, a] = await Promise.all([
              ...
              fetch(
                `/api/performance/actuals?userId=${u.uid}&period=${period}&periodType=${periodType}&role=am`,
                { credentials: 'include' },
              ).then((r) => r.json()),
            ]);
              ...
              {rows.map((row) => (
                <tr key={row.user.uid} className="border-t">
                  ...
                  <td>
                    <ProgressBar
                      label="Active Clients"
                      actual={Number(row.actuals.activeClients || 0)}
                      target={Number(row.metrics.activeClients?.target || 0)}
                      unit="count"
                      size="sm"
                    />
                  </td>
                  <td>
                    <ProgressBar
                      ...
                      actual={Number(row.actuals.projectsCompleted || 0)}
                      target={Number(row.metrics.projectsCompleted?.target || 0)}
                      unit="count"
                      size="sm"
                    />
                  </td>
