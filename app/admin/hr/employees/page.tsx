    ...
    setFormState({
      ...
      monthlyTarget:
        selectedUser.monthlyTarget !== undefined && selectedUser.monthlyTarget !== null
          ...
          : "",
      commission:
        selectedUser.commission !== undefined && selectedUser.commission !== null ? String(selectedUser.commission) : "",
  ...
  async function saveEdits() {
    ...
    try {
      ...
      const payload = {
        ...
        joiningDate: formState.joiningDate ? new Date(`${formState.joiningDate}T00:00:00.000Z`).toISOString() : null,
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 980 }}>
            <thead>
              <tr style={{ background: "var(--table-header-bg)" }}>
                ...
                <th style={headerCell} onClick={() => handleSort(setSortKey, setSortDir, sortKey, "department")}>Department</th>
                <th style={{ ...headerCell, textAlign: "right" }} onClick={() => handleSort(setSortKey, setSortDir, sortKey, "salary")}>
                  Salary PKR
                </th>
                <th style={{ ...headerCell, textAlign: "right" }} onClick={() => handleSort(setSortKey, setSortDir, sortKey, "target")}>
                  Target
                </th>
                <th style={{ ...headerCell, textAlign: "center" }} onClick={() => handleSort(setSortKey, setSortDir, sortKey, "status")}>
                  Status
                </th>
                <th style={{ ...headerCell, textAlign: "right" }} onClick={() => handleSort(setSortKey, setSortDir, sortKey, "joined")}>
                  Joined
                </th>
                ...
                sorted.map((user) => {
                  ...
                  return (
                    <tr
                      ...
                      <td style={{ ...cell, textAlign: "right" }}>{formatNumber(user.monthlyTarget)}</td>
                      <td style={{ ...cell, textAlign: "center" }}>{normalizeStatus(user.status) === "active" ? "Active" : "Inactive"}</td>
                      <td style={{ ...cell, textAlign: "right" }}>{formatDate(user.joiningDate)}</td>
                      <td style={{ ...cell, textAlign: "center" }}>
                        <button className="btn ghost" style={{ padding: "6px 14px", borderRadius: 999 }}>
                          View
                        </button>
                      </td>
                    </tr>
        ...
        >
          ...
          >
            <div style={{ display: "flex", justifyContent: "space-between", gap: 12 }}>
              <div>
                <div style={{ fontSize: 20, fontWeight: 900 }}>{selectedUser.name || "Employee"}</div>
              ...
              <div className="space-y-4">
                ...
                <div className="grid gap-3">
                  ...
                  <input
                    ...
                    onChange={(e) => setFormState((prev) => ({ ...prev, monthlyTarget: e.target.value }))}
                  />
                </div>
                <div className="grid gap-3">
                  ...
                  <input
                    ...
                    onChange={(e) => setFormState((prev) => ({ ...prev, commission: e.target.value }))}
                  />
                </div>
                <div className="grid gap-3">
            ...
            <div style={{ display: "flex", justifyContent: "space-between", gap: 8 }}>
              <div style={{ display: "flex", gap: 8 }}>
                {canAdmin && (
                  <button
                    className="btn ghost"
                    onClick={() => toggleDeactivate(formState.status === "inactive" ? "active" : "inactive")}
                    disabled={saving}
                    style={{
                      borderRadius: 12,
                      fontWeight: 600,
              ...
              <div style={{ display: "flex", gap: 8 }}>
                {!editMode ? (
                  <button className="btn" style={{ borderRadius: 12 }} onClick={() => setEditMode(true)}>
                    Edit
                  </button>
                ) : (
                  <>
