...
export default function HrOnboardingPage() {
  ...
  async function saveTemplate() {
    try {
      ...
      toastError(err?.message || 'Unable to save template.');
    }
  }
  ...
  async function assignTemplate() {
    try {
      ...
      toastError(err?.message || 'Unable to assign onboarding.');
    } finally {
  ...
  async function updateTask(task: TaskRecord) {
    try {
      ...
      toastError(err?.message || 'Unable to update task.');
    } finally {
      setTaskSaving(false);
    }
  }
                ...
                {templates.map((template) => (
                  ...
                  >
                    ...
                    <div style={{ fontSize: 12, color: 'var(--sidebar-text)' }}>
                      Role: {formatRole(template.role)} · {template.steps.length} steps ·{' '}
              ...
              {selectedTask.steps.map((step, idx) => (
                ...
                >
                  ...
                  <div>
                    ...
                    {step.required && (
                      <div style={{ fontSize: 11, color: 'var(--color-sky)' }}>Required</div>
                    )}
                  </div>
                </label>
              ))}
