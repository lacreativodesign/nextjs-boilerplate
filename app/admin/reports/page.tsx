    ...
    <div className="space-y-6">
      ...
      <section>
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <KpiCard
            ...
            value={formatUsd(overview.kpis.revenueThisMonth)}
            subtitle="Paid invoices"
          />
          <KpiCard
            title="Payments (This Month)"
            value={formatUsd(overview.kpis.paymentsThisMonth)}
            subtitle="Cash received"
          />
          <KpiCard
            title="Outstanding AR"
            value={formatUsd(overview.kpis.outstandingArTotal)}
            subtitle="Open invoices"
          />
          <KpiCard
            title="Overdue Invoices"
            value={`${overview.kpis.overdueInvoicesCount}`}
            subtitle="Past due"
          />
        </div>
      </section>
      ...
      <section>
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <KpiCard
            ...
            value={`${overview.ops.activeProjectsCount}`}
            subtitle="In flight"
          />
          <KpiCard
            title="Overdue Projects"
            value={`${overview.ops.overdueProjectsCount}`}
            subtitle="Needs attention"
          />
          <KpiCard title="QA Queue" value={`${overview.ops.qaQueueCount}`} subtitle="Final stage" />
          <KpiCard
            title="Open Change Requests"
            value={`${overview.ops.openChangeRequestsCount}`}
            subtitle="Pending"
          />
        </div>
      </section>
      ...
      <section>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <KpiCard
            ...
            value={`${overview.people.activeEmployeesCount}`}
            subtitle="Current users"
          />
          <KpiCard
            title="Onboarding Open"
            value={`${overview.people.onboardingOpenCount}`}
      ...
      <section className="grid gap-6 lg:grid-cols-2">
        <CardShell>
          ...
          <MiniBarChart
            rows={(overview.revenueSeries.length
              ? overview.revenueSeries
              : [{ label: '-', revenue: 0, payments: 0 }]
            ).map((row) => ({
              label: row.label,
              value: row.revenue,
            }))}
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p style={{ fontSize: 13, color: 'var(--sidebar-text)' }}>
              Live feed across finance, projects, production, and HR.
            </p>
          </div>
