...
export default function RevenueReportsPage() {
  ...
  const [sortKey, setSortKey] = useState<keyof RevenueResponse["outstandingInvoices"][number]>("dueDate");
  ...
  const outstandingTotal = data.outstandingInvoices.reduce((sum, row) => sum + row.amountTotalUsd, 0);
  ...
  return (
    <div className="space-y-6">
      ...
      <section>
        <div style={{ fontSize: 16, fontWeight: 700, marginBottom: 12 }}>Revenue & AR KPIs (USD)</div>
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          <KpiCard title="Outstanding AR" value={formatUsd(outstandingTotal)} subtitle="Open invoices" />
          <KpiCard title="Top Clients" value={`${data.topClientsByRevenue.length}`} subtitle="By paid revenue" />
        </div>
      </section>
      ...
      <section>
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p style={{ fontSize: 13, color: "var(--sidebar-text)" }}>Sync revenue KPIs with precise time windows.</p>
          </div>
          <div className="flex flex-wrap gap-2">
            ...
            <button className="btn ghost" onClick={() => downloadCsv("/api/admin/reports/exports/payments-by-month")} style={{ borderRadius: 999 }}>
              Download Payments by Month
            </button>
            <button className="btn ghost" onClick={() => downloadCsv("/api/admin/reports/exports/outstanding-ar")} style={{ borderRadius: 999 }}>
      ...
      <section className="grid gap-6 lg:grid-cols-2">
        <CardShell>
          ...
          <MiniBarChart
            rows={(data.revenueByMonth.length ? data.revenueByMonth : [{ label: "-", revenueUsd: 0 }]).map((row) => ({
              label: row.label,
        ...
        <CardShell>
          ...
          <MiniBarChart
            rows={(data.paymentsByMonth.length ? data.paymentsByMonth : [{ label: "-", paymentsUsd: 0 }]).map((row) => ({
              label: row.label,
      ...
      <section className="grid gap-6 lg:grid-cols-2">
        <CardShell>
          ...
          <MiniBarChart
            rows={(data.arAging.length ? data.arAging : [{ label: "-", amountUsd: 0 }]).map((row) => ({
              label: row.label,
              value: row.amountUsd,
            }))}
            valueFormatter={formatUsd}
        ...
        <CardShell>
          ...
          <MiniBarChart
            rows={(data.topClientsByRevenue.length ? data.topClientsByRevenue : [{ clientName: "-", totalUsd: 0 }]).map((row) => ({
              ...
              <thead>
                <tr style={{ background: "rgba(148,163,184,0.15)" }}>
                  <th style={{ textAlign: "left", padding: "14px 16px", fontWeight: 700 }}>
                    <button type="button" onClick={() => toggleSort("orderId")} style={{ background: "none", border: "none", fontWeight: 700 }}>
                      Invoice {sortKey === "orderId" ? (sortDirection === "asc" ? "▲" : "▼") : ""}
                    </button>
                  </th>
                  <th style={{ textAlign: "left", padding: "14px 16px", fontWeight: 700 }}>
                    <button type="button" onClick={() => toggleSort("clientName")} style={{ background: "none", border: "none", fontWeight: 700 }}>
                      Client {sortKey === "clientName" ? (sortDirection === "asc" ? "▲" : "▼") : ""}
                    </button>
                  </th>
                  <th style={{ textAlign: "right", padding: "14px 16px", fontWeight: 700 }}>
                    <button type="button" onClick={() => toggleSort("amountTotalUsd")} style={{ background: "none", border: "none", fontWeight: 700 }}>
                      Amount (USD) {sortKey === "amountTotalUsd" ? (sortDirection === "asc" ? "▲" : "▼") : ""}
                    </button>
                  </th>
                  <th style={{ textAlign: "right", padding: "14px 16px", fontWeight: 700 }}>
                    <button type="button" onClick={() => toggleSort("dueDate")} style={{ background: "none", border: "none", fontWeight: 700 }}>
                      Due Date {sortKey === "dueDate" ? (sortDirection === "asc" ? "▲" : "▼") : ""}
                    </button>
                  </th>
                  <th style={{ textAlign: "center", padding: "14px 16px", fontWeight: 700 }}>Status</th>
                </tr>
              </thead>
              <tbody>
                {loading ? (
                  ...
                  sortedInvoices.map((invoice, idx) => {
                    ...
                    return (
                      <tr key={invoice.id} style={{ background: rowBg }}>
                        <td style={{ padding: "14px 16px", textAlign: "left" }}>{invoice.orderId}</td>
                        ...
                        <td style={{ padding: "14px 16px", textAlign: "right" }}>{formatDate(invoice.dueDate)}</td>
                        <td style={{ padding: "14px 16px", textAlign: "center" }}>{invoice.status}</td>
                      </tr>
                    );
                  })
                )}
