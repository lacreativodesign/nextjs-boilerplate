    ...
    <div className="space-y-6">
      ...
      <section>
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          <KpiCard
            ...
            subtitle="Top 10 clients"
          />
          <KpiCard
            title="Revenue Tracked"
            value={formatUsd(data.clients.reduce((sum, row) => sum + row.totalPaidUsd, 0))}
            subtitle="Paid USD"
          />
        </div>
      </section>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p style={{ fontSize: 13, color: 'var(--sidebar-text)' }}>
              Segment coverage, key accounts, and change requests.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            ...
            <button
              className="btn ghost"
              onClick={() => downloadCsv('/api/admin/reports/exports/change-requests')}
              style={{ borderRadius: 999 }}
            >
      ...
      <section className="grid gap-6 lg:grid-cols-2">
        <CardShell>
          ...
          <MiniBarChart
            rows={(data.clientHealthDistribution.length
        ...
        <CardShell>
          ...
          <MiniBarChart
            rows={(data.segmentCoverage.length
      ...
      <section className="grid gap-6 lg:grid-cols-2">
        ...
        <CardShell>
          ...
          <MiniBarChart
            rows={(data.changeRequestsByClient.length
              ? data.changeRequestsByClient
              : [{ clientName: '-', count: 0 }]
            ).map((row) => ({
              ...
              <thead>
                <tr style={{ background: 'rgba(148,163,184,0.15)' }}>
                  ...
                  <th style={{ textAlign: 'right', padding: '14px 16px', fontWeight: 700 }}>
                    <button
                      type="button"
                      onClick={() => toggleSort('totalPaidUsd')}
                      style={{ background: 'none', border: 'none', fontWeight: 700 }}
                    >
                      Revenue (USD){' '}
                      {sortKey === 'totalPaidUsd' ? (sortDirection === 'asc' ? '▲' : '▼') : ''}
                    </button>
                  </th>
                  <th style={{ textAlign: 'right', padding: '14px 16px', fontWeight: 700 }}>
                    <button
                    ...
                    >
                      Active Projects{' '}
                      {sortKey === 'activeProjects' ? (sortDirection === 'asc' ? '▲' : '▼') : ''}
                    </button>
                  </th>
                  <th style={{ textAlign: 'right', padding: '14px 16px', fontWeight: 700 }}>
                    Last Activity
