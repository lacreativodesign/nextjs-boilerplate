...
export default function ProductionReportsPage() {
  ...
  const [sortKey, setSortKey] =
    useState<keyof ProductionResponse['stuckProjects'][number]>('ageDays');
  ...
  const totalOverdue = data.workloadByProductionOwner.reduce(
    (sum, owner) => sum + owner.overdue,
    0,
  );
    ...
    <div className="space-y-6">
      ...
      <section>
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          <KpiCard
            ...
            subtitle="Top stage clusters"
          />
          <KpiCard
            title="Stuck Projects"
            value={`${data.stuckProjects.length}`}
            subtitle="Beyond SLA"
          />
        </div>
      </section>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p style={{ fontSize: 13, color: 'var(--sidebar-text)' }}>
              Monitor production throughput and aging queues.
            </p>
          </div>
      ...
      <section className="grid gap-6 lg:grid-cols-2">
        <CardShell>
          ...
          <MiniBarChart
            rows={(data.workloadByProductionOwner.length
              ? data.workloadByProductionOwner
              : [{ productionOwnerId: '', productionOwnerName: '-', total: 0, overdue: 0 }]
            ).map((row) => ({
        ...
        <CardShell>
          ...
          <MiniBarChart
            rows={(data.queueAging.length ? data.queueAging : [{ stage: '-', avgDays: 0 }]).map(
              (row) => ({
                label: row.stage,
                value: row.avgDays,
              }),
      ...
      <section className="grid gap-6 lg:grid-cols-2">
        <CardShell>
          ...
          <MiniBarChart
            rows={(data.bottlenecks.length ? data.bottlenecks : [{ stage: '-', count: 0 }]).map(
              (row) => ({
                label: row.stage,
                value: row.count,
              }),
              ...
              <thead>
                <tr style={{ background: 'rgba(148,163,184,0.15)' }}>
                  ...
                  <th style={{ textAlign: 'right', padding: '14px 16px', fontWeight: 700 }}>
                    ...
                    >
                      Age (Days){' '}
                      {sortKey === 'ageDays' ? (sortDirection === 'asc' ? '▲' : '▼') : ''}
                    </button>
                  </th>
                  <th style={{ textAlign: 'right', padding: '14px 16px', fontWeight: 700 }}>
                    SLA (Days)
