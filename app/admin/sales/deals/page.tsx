...
export default function SalesDealsPage() {
  ...
  const { executeOptimisticDelete, executeOptimisticUpdate } =
    useOptimisticUpdate<DealRecord>(setDeals);
  ...
  const filteredDeals = useMemo(() => {
    ...
    return smartMatch(list, query, (deal) => [deal.dealName, deal.clientName, deal.ownerName]);
  ...
  const markClosed = async (deal: DealRecord, status: 'Closed Won' | 'Closed Lost') => {
    try {
      ...
      const optimisticDeals = deals.map((entry) =>
        entry.id === deal.id
          ? { ...entry, stage: status, updatedAt: new Date().toISOString() }
          : entry,
      );
