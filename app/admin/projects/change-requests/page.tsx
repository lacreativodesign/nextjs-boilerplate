...
export default function ChangeRequestsPage() {
  ...
  useEffect(() => {
    ...
    async function loadChangeRequests() {
      ...
      try {
        ...
        setError({
          title:
            isForbidden || isUnauthorized ? 'Access restricted' : 'Change requests can’t load yet',
          message:
            isForbidden || isUnauthorized
              ? 'You don’t have access to view change requests.'
  ...
  const filteredRows = useMemo(() => {
    ...
    list = smartMatch(list, query, (row) => [
      row.title,
      row.description,
      row.projectName,
      row.clientName,
  ...
  function openDrawer(row: ChangeRequestRecord) {
    ...
    setDrawerError(null);
    setCommercialCost(
      row.estimatedCost !== null && row.estimatedCost !== undefined
  ...
  return (
    <div>
      ...
      <div style={{ marginTop: 20 }}>
        {loading ? (
          <div style={{ padding: 30, color: 'var(--text-muted)' }}>Loading change requests...</div>
        ) : error ? (
          <div
            className="card"
            style={{
                ...
                <thead>
                  <tr>
                    ...
                    >
                      {headerLabel(
                        'Title',
                        sortKey === 'title' ? (sortDir === 'asc' ? '↑' : '↓') : undefined,
                      )}
                    </th>
                    <th
                      style={{ ...headerCellStyle, textAlign: 'left' }}
                    ...
                    >
                      {headerLabel(
                        'Project',
                        sortKey === 'projectName' ? (sortDir === 'asc' ? '↑' : '↓') : undefined,
                      )}
                    </th>
                    <th
                    ...
                    >
                      {headerLabel(
                        'Type',
                        sortKey === 'type' ? (sortDir === 'asc' ? '↑' : '↓') : undefined,
                      )}
                    </th>
                    <th
                      style={{ ...headerCellStyle, textAlign: 'center' }}
                    ...
                    >
                      {headerLabel(
                        'Status',
                        sortKey === 'status' ? (sortDir === 'asc' ? '↑' : '↓') : undefined,
                      )}
                    </th>
                    ...
                    >
                      {headerLabel(
                        'Priority',
                        sortKey === 'priority' ? (sortDir === 'asc' ? '↑' : '↓') : undefined,
                      )}
                    </th>
                    <th
                    ...
                    >
                      {headerLabel(
                        'Cost',
                        sortKey === 'estimatedCost' ? (sortDir === 'asc' ? '↑' : '↓') : undefined,
                      )}
                    </th>
                    <th
                      style={{ ...headerCellStyle, textAlign: 'right' }}
                    ...
                    >
                      {headerLabel(
                        'Updated At',
                        sortKey === 'updatedAt' ? (sortDir === 'asc' ? '↑' : '↓') : undefined,
                      )}
                    </th>
      ...
      {drawerOpen && selected && (
        <div className="drawer-overlay" onClick={closeDrawer}>
          <div className="drawer-panel drawer-panel--md" onClick={(e) => e.stopPropagation()}>
            ...
            {drawerError && (
              <div style={{ marginTop: 10, color: 'tomato', fontWeight: 600, fontSize: 13 }}>
                {drawerError}
              </div>
            )}
              ...
              <div style={{ display: 'grid', gap: 10 }}>
                ...
                <div style={{ display: 'grid', gap: 6 }}>
                  ...
                  <MasterSelect
                    ...
                    buttonStyle={{ height: 38, borderRadius: 10 }}
                    options={CHANGE_REQUEST_PRIORITIES.map((priority) => ({
                      label: priority,
                      value: priority,
                    }))}
                  />
