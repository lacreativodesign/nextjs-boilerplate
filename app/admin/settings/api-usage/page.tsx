  ...
  const load = async () => {
    try {
      ...
      if (!statsRes.ok || !statsJson.ok)
        throw new Error(statsJson.error || 'Failed loading usage stats');
      if (!logsRes.ok || !logsJson.ok)
        throw new Error(logsJson.error || 'Failed loading usage logs');
    ...
    <div className="space-y-6">
      ...
      <section className="grid gap-6 lg:grid-cols-2">
        <div className="card p-5 rounded-xl settings-section space-y-3">
          ...
          <input
            className="input"
            value={ruleForm.endpointPattern}
            onChange={(e) => setRuleForm({ ...ruleForm, endpointPattern: e.target.value })}
          ...
          <input
            ...
            onChange={(e) => setRuleForm({ ...ruleForm, limit: Number(e.target.value) })}
            placeholder="Limit"
          />
          <input
            className="input"
            ...
            onChange={(e) => setRuleForm({ ...ruleForm, windowSeconds: Number(e.target.value) })}
          ...
          <button className="btn" onClick={() => saveRule().catch((e) => setError(e.message))}>
            Save Rule
          </button>
        </div>
        ...
        <div className="card p-5 rounded-xl settings-section space-y-3">
          ...
          <input
            className="input"
            value={quotaForm.tenantId}
            onChange={(e) => setQuotaForm({ ...quotaForm, tenantId: e.target.value })}
            placeholder="Tenant ID"
          />
          <input
            className="input"
            value={quotaForm.plan}
            onChange={(e) => setQuotaForm({ ...quotaForm, plan: e.target.value })}
            placeholder="Plan"
          />
          <input
            className="input"
            ...
            onChange={(e) => setQuotaForm({ ...quotaForm, dailyLimit: Number(e.target.value) })}
          ...
          <input
            ...
            onChange={(e) => setQuotaForm({ ...quotaForm, monthlyLimit: Number(e.target.value) })}
            placeholder="Monthly limit"
          />
          <button className="btn" onClick={() => saveQuota().catch((e) => setError(e.message))}>
            Save Quota
          </button>
        </div>
      </section>
