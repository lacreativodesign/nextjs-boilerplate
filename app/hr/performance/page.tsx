...
export default function PerformancePage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`/api/performance/targets?period=${period}&periodType=${periodType}`, {
        credentials: 'include',
      }).then((r) => r.json()),
      fetch(`/api/performance/actuals?period=${period}&periodType=${periodType}&role=hr`, {
        credentials: 'include',
      }).then((r) => r.json()),
    ])
      .then(([t, a]) => {
  ...
  const getUserName = (uid: string) => users.find((u) => u.uid === uid)?.name || 'Unknown';
  ...
  const filteredUsers =
    selectedUser === 'all' ? users : users.filter((u) => u.uid === selectedUser);

  if (loading)
    return <div className="card p-6 text-[var(--text-muted)]">Loading performance data...</div>;
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="space-y-3">
        ...
        {targetsLoading ? (
        ...
        ) : Object.keys(myTargets).length === 0 ? (
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <PerformanceCard
              metricKey="employeesOnboarded"
              label="Employees Onboarded"
              actual={Number(myActuals.employeesOnboarded || 0)}
              target={Number(myTargets.employeesOnboarded?.target || 0)}
              unit="count"
            />
            <PerformanceCard
              metricKey="tasksCompleted"
              label="Tasks Completed"
              actual={Number(myActuals.tasksCompleted || 0)}
              target={Number(myTargets.tasksCompleted?.target || 0)}
              unit="count"
            />
          </div>
        )}
      ...
      <div className="flex gap-2 border-b border-[var(--border-subtle)] pb-0">
        ...
        <div className="ml-auto">
          <button onClick={() => setAddOpen(true)} className="btn text-sm px-4 py-2">
            + Set Target
          </button>
        </div>
      </div>
                ...
                >
                  ...
                  <div className="space-y-3">
                    ...
                    {!userReviews.length && !userTargets.length && (
                      <p className="text-sm text-[var(--text-muted)]">No performance data yet.</p>
                    )}
                  </div>
                </div>
              );
      ...
      {activeTab === 'department' && (
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          {deptStats.length === 0 && (
            <p className="text-[var(--text-muted)] col-span-2">No department data available.</p>
          )}
        </div>
      )}
      ...
      {activeTab === 'org' && (
        <div className="space-y-4">
          <div className="rounded-xl border border-[var(--border-subtle)] bg-[var(--surface-card)] p-6">
            <p className="font-bold text-[var(--text-primary)] mb-4">Organization Performance</p>
            ...
            <div className="flex justify-between items-center mb-6">
              <div>
                ...
                <p className="text-sm text-[var(--text-muted)]">
                  Assign a measurable KPI target to a user.
                </p>
              </div>
