...
export default function FinancePerformancePage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch(`/api/performance/actuals?period=${period}&periodType=${periodType}&role=finance`, {
        credentials: 'include',
      }).then((r) => r.json()),
    ])
      .then(([t, a]) => {
      ...
      <section className="space-y-3">
        ...
        {targetLoading ? (
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <PerformanceCard
              metricKey="invoicesCreated"
              label="Invoices Created"
              actual={Number(actuals.invoicesCreated || 0)}
              target={Number(targets.invoicesCreated?.target || 0)}
              unit="count"
            />
            <PerformanceCard
              metricKey="revenueInvoiced"
              label="Revenue Invoiced"
              actual={Number(actuals.revenueInvoiced || 0)}
              target={Number(targets.revenueInvoiced?.target || 0)}
              unit="USD"
            />
          </div>
        )}
