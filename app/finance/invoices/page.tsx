  ...
  return (
    <div>
      ...
      <div style={{ marginTop: 12 }}>
        <button
          className="btn"
          onClick={() => router.push('/admin/finance/invoices/create')}
      ...
      {drawerOpen && selectedInvoice && (
        <InvoiceDrawer
          ...
          actionLoading={
            actionLoading === selectedInvoice.id || actionLoading === `delete-${selectedInvoice.id}`
          }
        />
      )}
    </div>
  ...
  const paymentLink = canShowPaymentLink
    ? (() => {
        try {
          ...
          const t = invoice.paymentToken
            ? `?token=${encodeURIComponent(invoice.paymentToken)}`
            : '';
