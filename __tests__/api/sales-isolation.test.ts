...
const db = new FirestoreEmulator({
  deals: [
    { id: "deal_a", data: { tenantId: "tenant_a", ownerId: "sales_a", stage: "qualified", value: 100 } },
    { id: "deal_b", data: { tenantId: "tenant_b", ownerId: "sales_b", stage: "qualified", value: 200 } },
  ],
});

const requireSalesWrite = jest.fn(async () => ({
  ok: true as const,
  user: { uid: "sales_a", tenantId: "tenant_a", role: "sales_manager", name: "Sales A", email: "a@example.com" },
}));
...
describe("sales tenant isolation", () => {
  ...
  it("returns unauthorized on missing sales auth", async () => {
    requireSalesWrite.mockResolvedValueOnce({ ok: false, status: 401, error: "Unauthorized" } as any);
    const route = await import("@/app/api/sales/deals/update/route");
    const res = await route.POST(
