...
const cookiesGet = jest.fn((name: string) =>
  name === 'lac_session' ? { value: 'session_token' } : undefined,
);
...
describe('auth sessions API', () => {
  ...
  it('prevents invalidating current session by id', async () => {
    ...
    const res = await route.DELETE(
      jsonRequest('https://app.local/api/auth/sessions/sess_current', undefined, {
        method: 'DELETE',
      }) as any,
      {
  ...
  it('invalidates all sessions excluding current token', async () => {
    ...
    const res = await route.POST(
      jsonRequest('https://app.local/api/auth/sessions/invalidate-all', {}) as any,
    );
