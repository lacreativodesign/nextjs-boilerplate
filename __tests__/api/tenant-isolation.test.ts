...
const requireFinance = jest.fn(async () => ({
  ok: true,
  user: financeUser({ uid: 'finance_a', tenantId: 'tenant_a', role: 'finance' }),
}));
...
describe('tenant isolation and RBAC enforcement', () => {
  it('prevents tenant A from reading tenant B recurring history', async () => {
    ...
    const res = await route.GET(
      jsonRequest('https://app.local/api/finance/recurring-invoices/history?templateId=template_b'),
    );
