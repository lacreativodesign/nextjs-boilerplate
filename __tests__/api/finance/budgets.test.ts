...
describe("finance budgets API", () => {
  ...
  it("returns budget variance for tenant and blocks cross-tenant variance", async () => {
    ...
    const allowed = await GET(jsonRequest("https://app.local/api/finance/budgets/variance?budgetId=budget_a"));
    const allowedBody = await allowed.json();
    expect(allowed.status).toBe(200);
    expect(allowedBody.variance.variance).toBe(400);

    const forbidden = await GET(jsonRequest("https://app.local/api/finance/budgets/variance?budgetId=budget_b"));
    expect(forbidden.status).toBe(403);
  });

  it("denies deleting budget from another tenant", async () => {
    const { POST } = await import("@/app/api/finance/budgets/delete/route");
    const res = await POST(jsonRequest("https://app.local/api/finance/budgets/delete", { budgetId: "budget_b" }));
