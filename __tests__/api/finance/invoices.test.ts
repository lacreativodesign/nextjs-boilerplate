...
jest.mock("@/lib/security", () => ({ checkRateLimit: jest.fn(async () => undefined), getClientIp: jest.fn(() => "127.0.0.1") }));
...
jest.mock("@/lib/finance/exchangeRates", () => ({ getExchangeRate: jest.fn(async () => 1.1), storeHistoricalRate: jest.fn() }));
jest.mock("@/lib/finance/tax", () => ({ calculateTax: jest.fn(() => ({ amountTax: 0, total: 100 })) }));
