...
**Baseline:** measured at `f0d3ce4`; status re-checked at `94caba7` (see Post-audit status below).
...
Still open and unchanged at `94caba7`: `tabular-nums` is used 0 times, `<Breadcrumbs>` is rendered
by 0 files, and the four `duration-*` class values are untouched.
