...
| ---- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------ | --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| F-1  | `app/login/page.tsx:451–1002`                                                                                                                                                                                                                                                                                                                                                                                                                            | dead/duplicate CSS | P0  | A 551-line `<style jsx global>` block defining `.login-*`. It duplicates root `styles.css`, which is **never imported** — so the login page is the _only_ thing keeping these classes alive, and 23 raw `rgba()` literals with it.                                                                                                                                                          | Move to `app/globals.css` behind tokens; delete root `styles.css`.                                                                                                       |
| F-2  | `styles.css` (409 lines, 37 selectors), `styles/globals.css` (33 lines)                                                                                                                                                                                                                                                                                                                                                                                  | dead code          | P1  | Neither file is imported anywhere (`app/layout.tsx:1` imports only `./globals.css`). `styles.css` defines a **conflicting `.btn` and `.btn-primary`** at lines 352/369.                                                                                                                                                                                                                     | Delete both files.                                                                                                                                                       |
| F-3  | `app/globals.css` vs `styles/globals.css`                                                                                                                                                                                                                                                                                                                                                                                                                | orphan tokens      | P2  | `--header-bg`, `--header-text`, `--sidebar-bg` are defined only in the dead sheet and referenced by zero components.                                                                                                                                                                                                                                                                        | Remove; `Header.tsx` already uses `--surface-card` / `--text-primary`.                                                                                                   |
...
| F-26 | `app/admin/finance/page.tsx:210` and `:216`                                                                                                                                                                                                                                                                                                                                                                                                              | loading state      | P2  | Dead branch — the whole subtree is already inside `{loading ? <SkeletonDashboard/> : (…)}` at line 82, so this inner `{loading ? 'Loading activity…' : …}` can never render its loading arm.                                                                                                                                                                                                | Remove the inner ternary.                                                                                                                                                |
...
| F-28 | 57 table pages with bare-text empty states (`app/admin/leads/page.tsx:196`, `app/admin/finance/invoices/page.tsx:391`, `app/admin/finance/page.tsx:222`, `app/sales/deals/page.tsx`, …)                                                                                                                                                                                                                                                                  | empty state        | P1  | Plain `No leads found.` / `No invoices found.` in a `<td>` — no icon, no guidance, no primary action.                                                                                                                                                                                                                                                                                       | `<EmptyState>`, or at minimum `.table-empty` which is already defined.                                                                                                   |
...
| F-30 | `components/support/BugReportButton.tsx:235`                                                                                                                                                                                                                                                                                                                                                                                                             | a11y               | P0  | Icon-only close button — `<button onClick={close} className="flex h-8 w-8 …"><X className="h-4 w-4"/></button>` — with no `aria-label`, `title`, or `sr-only` text. Screen readers announce "button".                                                                                                                                                                                       | `aria-label="Close bug report"`. (This is the **only** unlabelled icon-only button in the repo — `Sidebar.tsx:155,171` and `SmartSearchBar.tsx` are correctly labelled.) |
| F-31 | `tabular-nums` appears **0 times** repo-wide; 47 currency/number `<td>` cells across 31 files                                                                                                                                                                                                                                                                                                                                                            | numeric            | P1  | Proportional digits make currency columns visually ragged; only 13 of the 47 are even right-aligned.                                                                                                                                                                                                                                                                                        | `font-variant-numeric: tabular-nums` on `.table-cell-right`, and right-align all numeric columns.                                                                        |
...
| F-34 | `components/layout/Breadcrumbs.tsx`                                                                                                                                                                                                                                                                                                                                                                                                                      | navigation         | P0  | The component is fully implemented (with `aria-label="Breadcrumb"`, a mobile trail, and `getBreadcrumbs()` in `lib/navigation/sidebarConfig.ts`) and **is imported by nothing**. No page on the platform shows breadcrumbs.                                                                                                                                                                 | Render in `AppShell` above `{children}`, or in `Header`.                                                                                                                 |
...
| F-41 | `tailwind.config.js:10`                                                                                                                                                                                                                                                                                                                                                                                                                                  | config             | P2  | `theme.extend` is `{}`, so no token is reachable as a Tailwind utility. Every token use costs an arbitrary-value bracket (`bg-[var(--surface-card)]`), which is what makes hardcoding `bg-gray-50` the path of least resistance.                                                                                                                                                            | Map the tokens into `theme.extend.colors` so `bg-surface-card` works.                                                                                                    |
| F-42 | Durations: `duration-300` (6), `duration-500` (4), `duration-200` (3), `duration-700` (1), plus inline `0.2s`, `0.25s`, `0.55s`, and 14 distinct values in CSS (§6.3)                                                                                                                                                                                                                                                                                    | motion             | P2  | 18 distinct durations for a system with three interaction speeds.                                                                                                                                                                                                                                                                                                                           | Three tokens: `--motion-fast: 140ms`, `--motion-base: 160ms`, `--motion-slow: 300ms`.                                                                                    |
...
| 3    | **Render `<Breadcrumbs>` in `AppShell`** above `{children}` (F-34).                                                                                                                                                    |   all 259 | XS     | The component, its config (`getBreadcrumbs`), and its a11y are already written and tested-shaped. One import + one line restores navigation context platform-wide.                                        |
...
| 6    | **Delete `styles.css` + `styles/globals.css`, fold `.login-*` into `app/globals.css`** (F-1/2/3).                                                                                                                      | 1 + build | S      | Removes 993 lines of dead or page-trapped CSS, a conflicting `.btn` definition, 3 orphan tokens, and 23 of the repo's raw `rgba()` literals in one commit. Zero runtime risk — the files are unreachable. |
...
| 8    | **Add `tabular-nums` to `.table-cell-right` and right-align the 34 unaligned numeric columns** (F-31), and consolidate the 9 local currency formatters (F-32/33).                                                      |        31 | S      | A two-line CSS change fixes digit jitter in every money column at once; the formatter consolidation removes a real correctness bug (`$1234.56` on the invoices page).                                     |
...
| 10   | **Map tokens into `tailwind.config.js` `theme.extend.colors`** (F-41), then extract `<TablePager>` from `app/super_admin/payments/page.tsx` (F-36).                                                                    |       all | S/M    | The config change is preventive: until `bg-surface-card` works, `bg-gray-50` will keep winning on ergonomics. The pager unblocks the 89 unpaginated tables.                                               |
...
Surrounding 15 lines, verbatim (lines 120–134):
...
Related references, if this entry is ever removed:
...
- i18n key `navigation.supportTickets`

**Not removed in this session** — this audit is read-only.
...
| (i) icon-only buttons missing `aria-label`                                   | **1** — `components/support/BugReportButton.tsx:235`        |
| (j) `tabular-nums`                                                           | **0** uses; 47 numeric `<td>` in 31 files, 13 right-aligned |
| (k) pages rendering `<Breadcrumbs>`                                          | **0** — component exists, imported by nothing               |
| (l) distinct transition durations                                            | 18                                                          |

---

_Read-only audit. No `.tsx`, `.ts`, or `.css` file was modified._
